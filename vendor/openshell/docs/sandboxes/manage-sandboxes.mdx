...
Docker gateways can opt in to image-declared FUSE filesystems. The operator
must set `enable_fuse = true` in `[openshell.drivers.docker]`, the sandbox image
policy must declare the daemon and mountpoint, and the create request must
explicitly include `--fuse`:

```shell
openshell sandbox create --from example.com/team/fuse-sandbox:v1 --fuse
```

The request cannot supply a daemon path, mountpoint, or mount options. Those
values are immutable image policy so an untrusted caller cannot turn the
capability into an arbitrary mount primitive. OpenShell opens `/dev/fuse`,
mounts the declared path before applying the supervisor seccomp prelude, and
passes only the resulting FUSE file descriptor to the declared daemon.

FUSE is currently supported only by the Docker driver. Podman, Kubernetes,
VM, and extension drivers reject a request containing `--fuse`. A Docker
gateway also rejects the request when `/dev/fuse` is unavailable or the
