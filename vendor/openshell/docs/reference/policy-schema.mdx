...
| `fuse_mounts` | list of objects | No | Static | Declares an image-owned FUSE daemon and mountpoint. |
...
Declares an image-owned FUSE daemon that the supervisor starts as a critical
child. A public sandbox request can ask for this capability with `--fuse`, but
cannot override any field in this policy entry.

| Field | Type | Required | Description |
|---|---|---|---|
| `binary` | string | Yes | Absolute path to the root-owned daemon executable. It must be outside the sandbox workspace. |
| `args` | list of strings | No | Arguments passed after `binary`. |
| `mountpoint` | string | Yes | Absolute mountpoint strictly below the resolved sandbox workspace. |
| `fs_name` | string | No | Conservative filesystem name reported by the kernel. |
| `allow_other` | bool | No | Allows users other than the daemon owner to access the mount. Defaults to `false`. |
| `startup_timeout_seconds` | integer | No | Time to wait for the daemon's FUSE INIT readiness signal. Must be in the accepted bounded range. |

**Validation and runtime constraints:**

- At most one FUSE mount is accepted in the current schema.
- `binary` and `mountpoint` must be normalized absolute paths without `..`.
- `binary` must be outside the workspace; `mountpoint` must be a child of it.
- The daemon must be a root-owned, non-group-writable, non-world-writable
  regular executable without symlink path components.
- FUSE policy is immutable. `policy update` cannot add, remove, or change it.
- The capability remains inactive unless the Docker operator enables FUSE and
  the create request includes `--fuse`.

Example:

```yaml showLineNumbers={false}
fuse_mounts:
  - binary: /usr/local/bin/example-fused
    args: [serve]
