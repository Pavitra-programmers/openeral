...
`--fuse` is a separate, default-off resource request supported only by the
Docker driver. The Docker gateway must set `enable_fuse = true`, the host must
provide `/dev/fuse`, and the selected image policy must declare exactly one
FUSE daemon and mountpoint. Other drivers reject the request instead of
silently running without the requested filesystem.
...
### Docker FUSE Capability

FUSE support is disabled by default. Enable it only on a gateway host where
`/dev/fuse` exists:
...
Creating a FUSE sandbox then requires all three independent grants:

1. The operator enables FUSE in Docker driver configuration.
2. The image policy declares one immutable `fuse_mounts` entry.
3. The caller requests the resource with `openshell sandbox create --fuse`.

The Docker driver maps exactly `/dev/fuse` with `rwm` access. It does not expose
arbitrary devices or accept caller-provided mount details. The supervisor
validates the root-owned daemon binary and mountpoint, mounts before its TSYNC
seccomp prelude blocks further mount syscalls, and starts the daemon through
the normal sandbox security path.

A FUSE sandbox uses Docker's `on-failure` restart policy with at most five
restarts. A critical FUSE daemon exit terminates the supervisor with a reserved
failure status so Docker rebuilds the container mount namespace. OpenShell
reports intermediate restarts as transient; exhausting the restart budget is
terminal. Non-FUSE sandboxes retain the standard `unless-stopped` policy.
