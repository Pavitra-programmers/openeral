...
# Default-off operator grant for image-declared FUSE sandboxes. Requires a
# Linux Docker host with /dev/fuse. Callers must also pass --fuse and select an
# image whose static policy declares a FUSE daemon and mountpoint.
enable_fuse        = false
