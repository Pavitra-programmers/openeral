...
use crate::proto::{
    FilesystemPolicy as ProtoFilesystemPolicy, FuseMount as ProtoFuseMount,
    LandlockPolicy as ProtoLandlockPolicy, ProcessPolicy as ProtoProcessPolicy,
...
pub struct SandboxPolicy {
    ...
    pub fuse_mounts: Vec<FuseMountPolicy>,
}
    ...
    fn try_from(proto: ProtoSandboxPolicy) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            fuse_mounts: proto
                .fuse_mounts
                .into_iter()
                .map(FuseMountPolicy::from)
                .collect(),
        })
    }
}

impl From<ProtoFuseMount> for FuseMountPolicy {
    fn from(proto: ProtoFuseMount) -> Self {
        Self {
            binary: PathBuf::from(proto.binary),
            args: proto.args,
            mountpoint: PathBuf::from(proto.mountpoint),
            fs_name: proto.fs_name,
            allow_other: proto.allow_other,
            startup_timeout_seconds: if proto.startup_timeout_seconds == 0 {
                30
            } else {
                proto.startup_timeout_seconds.clamp(1, 120)
            },
        }
    }
}
