...
| `restart_policy` | Uses `unless-stopped` normally. An explicitly requested FUSE sandbox uses `on-failure` with at most five retries so a critical daemon crash rebuilds the mount namespace without creating an unbounded crash loop. |
...
FUSE is an explicit, default-off Docker capability. Set `enable_fuse = true`
in the Docker driver configuration only when the gateway host has `/dev/fuse`.
The create request must include `--fuse`, and the image's immutable sandbox
policy must contain exactly one `fuse_mounts` declaration. The caller cannot
provide or override the daemon path, arguments, mountpoint, or filesystem name.

For an accepted request the driver maps only `/dev/fuse` into the container.
The supervisor opens the device, validates and prepares the declared mountpoint,
performs the mount before installing its TSYNC seccomp prelude, and passes the
FUSE descriptor to the validated daemon through an explicit inherited-FD
mapping. Agent processes remain unable to call `mount` or `umount2`.

The FUSE daemon is a critical supervisor child. Its unexpected exit terminates
the supervisor with the reserved restart status, after which Docker reconstructs
the whole container and mount namespace. Restarting in place is intentionally
not attempted because the supervisor seccomp prelude blocks later mount calls.
