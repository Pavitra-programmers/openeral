...
use openshell_core::proto::compute::v1::{
    DriverResourceRequirements, DriverSandboxSpec, DriverSandboxTemplate, FuseResourceRequirements,
...
fn fuse_resources() -> ResourceRequirements {
    ResourceRequirements {
        fuse: Some(FuseResourceRequirements {}),
        ..Default::default()
    }
}
...
#[test]
fn docker_compute_config_disables_fuse_by_default() {
    assert!(!DockerComputeConfig::default().enable_fuse);
}

#[test]
fn fuse_request_requires_operator_gate_and_host_device() {
    let disabled = validate_fuse_request(true, false, true).unwrap_err();
    assert_eq!(disabled.code(), tonic::Code::FailedPrecondition);
    assert!(disabled.message().contains("enable_fuse = true"));

    let missing = validate_fuse_request(true, true, false).unwrap_err();
    assert_eq!(missing.code(), tonic::Code::FailedPrecondition);
    assert!(missing.message().contains("/dev/fuse"));
    validate_fuse_request(false, false, false).unwrap();
    validate_fuse_request(true, true, true).unwrap();
}

#[test]
fn fuse_container_gets_only_fuse_device_and_bounded_restart_policy() {
    let mut sandbox = test_sandbox();
    sandbox.spec.as_mut().unwrap().resource_requirements = Some(fuse_resources());
    let mut config = runtime_config();
    config.enable_fuse = true;
    ...
    assert_eq!(host.devices.as_ref().map(Vec::len), Some(1));
    let device = &host.devices.unwrap()[0];
    ...
    let restart = host.restart_policy.expect("restart policy");
    assert_eq!(restart.name, Some(RestartPolicyNameEnum::ON_FAILURE));
    assert_eq!(restart.maximum_retry_count, Some(FUSE_MAX_RESTARTS));
}

#[test]
fn fuse_exit_classification_preserves_retry_and_crash_loop_boundary() {
    assert_eq!(
        fuse_exit_condition(i64::from(CRITICAL_SERVICE_EXIT_CODE), 0).0,
        "CriticalServiceRestarting"
    );
    assert_eq!(
        fuse_exit_condition(2, FUSE_MAX_RESTARTS - 1).0,
        "SandboxProcessRestarting"
    );
    assert_eq!(
        fuse_exit_condition(i64::from(CRITICAL_SERVICE_EXIT_CODE), FUSE_MAX_RESTARTS).0,
        "CriticalServiceCrashLoop"
    );
    assert_eq!(
        fuse_exit_condition(2, FUSE_MAX_RESTARTS).0,
        "SandboxProcessCrashLoop"
    );
    assert_eq!(fuse_exit_condition(0, 0).0, "ContainerExited");
}
