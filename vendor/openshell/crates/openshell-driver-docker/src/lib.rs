...
use bollard::models::{
    ...
    ContainerSummaryStateEnum, CreateImageInfo, DeviceMapping, DeviceRequest, EndpointSettings,
...
use openshell_core::driver_utils::{
    CRITICAL_SERVICE_EXIT_CODE, LABEL_MANAGED_BY, LABEL_MANAGED_BY_VALUE, LABEL_SANDBOX_ID,
...
const LABEL_FUSE_REQUESTED: &str = "com.nvidia.openshell.fuse-requested";
const FUSE_MAX_RESTARTS: i64 = 5;
...
pub struct DockerComputeConfig {
    ...
    /// Allow explicitly requested, image-policy-declared FUSE sandboxes.
    #[serde(default)]
...
impl DockerComputeDriver {
    pub async fn new(config: &Config, docker_config: &DockerComputeConfig) -> CoreResult<Self> {
        ...
        let driver = Self {
            ...
            config: DockerDriverRuntimeConfig {
                ...
                enable_fuse: docker_config.enable_fuse,
            },
    ...
    fn validated_sandbox<'a>(
        ...
        let fuse_requested = spec
            .resource_requirements
            .as_ref()
            .is_some_and(|requirements| requirements.fuse.is_some());
        validate_fuse_request(
            fuse_requested,
            config.enable_fuse,
    ...
    async fn get_sandbox_snapshot(
        ...
        let container = self
            ...
            .await?;
        if let Some(sandbox) = match container {
            Some(summary) => self.sandbox_from_container_summary(&summary).await,
            None => None,
        } {
    ...
    async fn current_snapshots(&self) -> Result<Vec<DriverSandbox>, Status> {
        ...
        for container in &containers {
            if let Some(sandbox) = self.sandbox_from_container_summary(container).await {
    ...
    async fn publish_container_snapshot(
        ...
        if let Some(summary) = self
            .find_managed_container_summary(sandbox_id, sandbox_name)
            .await?
            && let Some(sandbox) = self.sandbox_from_container_summary(&summary).await
        {
    ...
    async fn sandbox_from_container_summary(
        &self,
        summary: &ContainerSummary,
    ) -> Option<DriverSandbox> {
        let mut sandbox = sandbox_from_container_summary(summary)?;
        if summary.state != Some(ContainerSummaryStateEnum::EXITED)
            || !summary_requests_fuse(summary)
        {
            return Some(sandbox);
        }

        let Some(target) = summary_container_target(summary) else {
            return Some(sandbox);
        };
        let condition = match self.docker.inspect_container(&target, None).await {
            Ok(inspected) => {
                let exit_code = inspected
                    ...
                    .unwrap_or_default();
                fuse_exit_condition(exit_code, inspected.restart_count.unwrap_or_default())
            }
            Err(error) => {
                debug!(container = %target, error = %error, "Could not inspect exited FUSE sandbox");
                (
                    "InspectFailed",
                    "Could not inspect exited FUSE sandbox; retrying status reconciliation",
                )
            }
        };
        set_ready_condition(&mut sandbox, condition.0, condition.1);
        Some(sandbox)
    }
...
fn validate_fuse_request(
    requested: bool,
    operator_enabled: bool,
    host_device_available: bool,
) -> Result<(), Status> {
    if !requested {
        return Ok(());
    }
    if !operator_enabled {
        return Err(Status::failed_precondition(
            "FUSE sandboxes require enable_fuse = true in [openshell.drivers.docker]",
        ));
    }
    if !host_device_available {
        return Err(Status::failed_precondition(
            "FUSE was requested but /dev/fuse is unavailable on the Docker gateway host",
        ));
    }
    Ok(())
}
...
fn build_environment_for_oci_user(
    ...
    if let Some(spec) = sandbox.spec.as_ref() {
        ...
        user_env.remove(openshell_core::sandbox_env::FUSE_REQUESTED);
        user_env.retain(|key, _| !key.starts_with(openshell_core::sandbox_env::FUSE_FD_PREFIX));
    ...
    {
        environment.insert(
            openshell_core::sandbox_env::FUSE_REQUESTED.to_string(),
            "1".to_string(),
        );
    } else {
        environment.remove(openshell_core::sandbox_env::FUSE_REQUESTED);
    }
    environment.insert(
...
fn build_container_create_body_for_image(
    ...
    if fuse_requested {
        labels.insert(LABEL_FUSE_REQUESTED.to_string(), "true".to_string());
    }
    ...
    Ok(ContainerCreateBody {
        ...
        host_config: Some(HostConfig {
            ...
            restart_policy: Some(if fuse_requested {
                RestartPolicy {
...
fn summary_requests_fuse(summary: &ContainerSummary) -> bool {
    summary
        .labels
        .as_ref()
        .and_then(|labels| labels.get(LABEL_FUSE_REQUESTED))
        .is_some_and(|value| value == "true")
}

fn fuse_exit_condition(exit_code: i64, restart_count: i64) -> (&'static str, &'static str) {
    if exit_code == 0 {
        return (
            "ContainerExited",
            "FUSE sandbox exited unexpectedly with status 0",
        );
    }
    let critical = exit_code == i64::from(CRITICAL_SERVICE_EXIT_CODE);
    if restart_count >= FUSE_MAX_RESTARTS {
        if critical {
            (
                "CriticalServiceCrashLoop",
                "Critical FUSE service exhausted its Docker restart budget",
            )
        } else {
            (
                "SandboxProcessCrashLoop",
                "FUSE sandbox supervisor exhausted its Docker restart budget",
            )
        }
    } else if critical {
        (
            "CriticalServiceRestarting",
            "Critical FUSE service exited; Docker restart is pending",
        )
    } else {
        (
            "SandboxProcessRestarting",
            "FUSE sandbox supervisor exited; Docker restart is pending",
        )
    }
}

fn set_ready_condition(sandbox: &mut DriverSandbox, reason: &str, message: &str) {
