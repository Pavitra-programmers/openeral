...
/// Validate that static policy fields (filesystem, landlock, process, FUSE mounts) haven't changed
...
pub(super) fn validate_static_fields_unchanged(
    ...
    if baseline.fuse_mounts != new.fuse_mounts {
        return Err(Status::invalid_argument(
            "FUSE mounts cannot be changed on a live sandbox (mounted at startup)",
        ));
    }
    Ok(())
}
