...
use openshell_core::proto::compute::v1::{
    ...
    DriverSandboxTemplate, FuseResourceRequirements as DriverFuseResourceRequirements,
...
fn driver_sandbox_spec_from_public(
    ...
    if spec
        ...
        && driver_name != ComputeDriverKind::Docker.as_str()
    {
        return Err(Box::new(Status::failed_precondition(format!(
            "FUSE sandboxes are supported only by the Docker compute driver in v1; selected driver is '{driver_name}'"
        ))));
    }
        ...
        resource_requirements: spec.resource_requirements.as_ref().map(|requirements| {
            DriverSandboxResourceRequirements {
                ...
                fuse: requirements
                    .fuse
                    .as_ref()
                    .map(|_| DriverFuseResourceRequirements {}),
            }
        }),
