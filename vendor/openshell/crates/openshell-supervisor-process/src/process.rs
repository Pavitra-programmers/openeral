...
const SUPERVISOR_ONLY_ENV_VARS: &[&str] = &[
    ...
    openshell_core::sandbox_env::FUSE_REQUESTED,
];

pub fn is_supervisor_only_env_var(key: &str) -> bool {
    SUPERVISOR_ONLY_ENV_VARS.contains(&key)
        || key.starts_with(openshell_core::sandbox_env::FUSE_FD_PREFIX)
}
...
fn strip_supervisor_only_env(cmd: &mut Command) {
    ...
    for (key, _) in std::env::vars() {
        if key.starts_with(openshell_core::sandbox_env::FUSE_FD_PREFIX) {
            cmd.env_remove(key);
        }
    }
}
...
/// A supervisor-owned descriptor intentionally inherited by one child.
#[cfg(target_os = "linux")]
...
#[cfg(target_os = "linux")]
impl InheritedFd {
    /// Duplicate `source` to a currently unused descriptor at or above `floor`.
    /// The child receives the numeric descriptor through `env_key`.
    pub fn duplicate_at_least(
        source: OwnedFd,
        floor: RawFd,
        ...
        if floor < 64 {
            return Err(std::io::Error::new(
                ...
                "inherited descriptor floor must be at least 64",
            ));
        }
        let env_key = env_key.into();
        if !env_key.starts_with(openshell_core::sandbox_env::FUSE_FD_PREFIX) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "inherited descriptor environment key is not reserved for FUSE",
            ));
        }
        #[allow(unsafe_code)]
        let duplicated = unsafe { libc::fcntl(source.as_raw_fd(), libc::F_DUPFD_CLOEXEC, floor) };
        if duplicated < 0 {
...
impl ProcessHandle {
    ...
    /// Spawn a Linux child with supervisor-owned inherited descriptors.
    #[cfg(target_os = "linux")]
    #[allow(clippy::too_many_arguments)]
    ...
    fn spawn_impl(
        ...
        let mut inherited_targets = std::collections::HashSet::new();
        let mut inherited_env_keys = std::collections::HashSet::new();
        for mapping in inherited_fds {
            let target = mapping.target();
            if !inherited_targets.insert(target) {
                return Err(miette::miette!(
                    "duplicate inherited child descriptor target {target}"
                ));
            }
            if !inherited_env_keys.insert(mapping.env_key.as_str()) {
                return Err(miette::miette!(
                    "duplicate inherited descriptor environment key {}",
                    mapping.env_key
                ));
            }
            cmd.env(&mapping.env_key, target.to_string());
        }
        ...
        // setpgid and setns are async-signal-safe and safe to call in this context.
        {
            ...
            let inherited_targets = inherited_targets.into_iter().collect::<Vec<_>>();
...
mod tests {
    ...
    #[test]
    #[cfg(target_os = "linux")]
    fn inherited_fd_rejects_unsafe_floor_and_unreserved_environment_key() {
        let low_floor = InheritedFd::duplicate_at_least(
            tempfile::tempfile().unwrap().into(),
            63,
            format!("{}0", openshell_core::sandbox_env::FUSE_FD_PREFIX),
        );
        assert!(low_floor.is_err());

        let unreserved_key = InheritedFd::duplicate_at_least(
            tempfile::tempfile().unwrap().into(),
            200,
            "UNRESERVED_FD",
        );
        assert!(unreserved_key.is_err());
    }

    #[test]
    #[cfg(target_os = "linux")]
    fn inherited_fd_spawn_rejects_duplicate_environment_keys() {
        let env_key = format!("{}0", openshell_core::sandbox_env::FUSE_FD_PREFIX);
        let first = InheritedFd::duplicate_at_least(
            tempfile::tempfile().unwrap().into(),
            200,
            env_key.clone(),
        )
        .unwrap();
        let second =
            InheritedFd::duplicate_at_least(tempfile::tempfile().unwrap().into(), 200, env_key)
                .unwrap();
        let policy = policy_with_process(ProcessPolicy::default());

        let result = ProcessHandle::spawn_with_inherited_fds(
            "/bin/true",
            &[],
        ...
        let error = result.err().expect("duplicate key must be rejected");
        assert!(
            error
                .to_string()
                .contains("duplicate inherited descriptor environment key")
        );
    }
