...
pub async fn run_process(
    ...
    #[cfg(target_os = "linux")]
    let prepared_fuse = crate::fuse::prepare(
        policy,
        &workspace,
        resolved_process_identity,
        ssh_socket_path.as_deref().map(std::path::Path::new),
    )?;
    ...
    #[cfg(target_os = "linux")]
    let mut fuse_daemon = if let Some(prepared) = prepared_fuse {
        Some(
            prepared
                .spawn(
                    policy,
    ...
    #[cfg(target_os = "linux")]
    let critical_handle = fuse_daemon.as_mut().map(|daemon| &mut daemon.handle);
    #[cfg(not(target_os = "linux"))]
    let critical_handle = None;
    ...
    let status = match outcome {
        ...
        ProcessWaitOutcome::CriticalServiceExited(status) => {
            ocsf_emit!(
                ProcessActivityBuilder::new(ocsf_ctx())
                    .activity(ActivityId::Close)
                    .action(ActionId::Denied)
                    ...
                    .exit_code(status.code())
                    .message(format!(
                        "Critical FUSE service exited with code {}",
                        status.code()
                    ))
                    .build()
            );
            return Ok(openshell_core::driver_utils::CRITICAL_SERVICE_EXIT_CODE);
        }
    };
...
enum ProcessWaitOutcome {
    ...
    CriticalServiceExited(ProcessStatus),
}

async fn wait_for_process_exit_or_shutdown(
    handle: &mut ProcessHandle,
    critical_handle: Option<&mut ProcessHandle>,
    timeout_secs: u64,
    terminating: &AtomicBool,
) -> Result<ProcessWaitOutcome> {
    if let Some(critical_handle) = critical_handle {
        return wait_for_process_exit_or_shutdown_with_critical(
            handle,
            critical_handle,
            timeout_secs,
            terminating,
...
async fn wait_for_process_exit_or_shutdown_with_critical(
    handle: &mut ProcessHandle,
    critical_handle: &mut ProcessHandle,
    timeout_secs: u64,
    terminating: &AtomicBool,
) -> Result<ProcessWaitOutcome> {
    let workload_pid = handle.pid();
    let critical_pid = critical_handle.pid();
    let workload_wait = handle.wait();
    let critical_wait = critical_handle.wait();
    tokio::pin!(workload_wait);
    tokio::pin!(critical_wait);
    ...
    tokio::select! {
        result = &mut workload_wait => {
            terminating.store(true, Ordering::Release);
            signal_pid(critical_pid, nix::sys::signal::Signal::SIGTERM, "workload exit");
            if tokio::time::timeout(Duration::from_secs(10), &mut critical_wait).await.is_err() {
                signal_pid(critical_pid, nix::sys::signal::Signal::SIGKILL, "critical service shutdown timeout");
            }
            Ok(ProcessWaitOutcome::Exited(result.into_diagnostic()?))
        }
        result = &mut critical_wait => {
            terminating.store(true, Ordering::Release);
            let status = result.into_diagnostic()?;
            terminate_then_kill_pid(workload_pid).await;
            Ok(ProcessWaitOutcome::CriticalServiceExited(status))
        }
        () = &mut timeout => {
            terminating.store(true, Ordering::Release);
            terminate_then_kill_pid(workload_pid).await;
            terminate_then_kill_pid(critical_pid).await;
            Ok(ProcessWaitOutcome::TimedOut)
        }
        signal = wait_for_supervisor_shutdown_signal() => {
            terminating.store(true, Ordering::Release);
            signal_entrypoint_for_shutdown(workload_pid, signal);
            signal_pid(critical_pid, nix::sys::signal::Signal::SIGTERM, signal);
            let status = (&mut workload_wait).await.into_diagnostic()?;
            if tokio::time::timeout(Duration::from_secs(10), &mut critical_wait).await.is_err() {
                signal_pid(critical_pid, nix::sys::signal::Signal::SIGKILL, "critical service shutdown timeout");
            }
            Ok(ProcessWaitOutcome::ShutdownSignal { signal, status })
