...
fn to_proto(raw: PolicyFile) -> Result<SandboxPolicy> {
    ...
    Ok(SandboxPolicy {
        ...
        fuse_mounts: raw
            .fuse_mounts
            .into_iter()
            .map(|mount| openshell_core::proto::FuseMount {
                binary: mount.binary,
                args: mount.args,
                mountpoint: mount.mountpoint,
                fs_name: mount.fs_name,
                allow_other: mount.allow_other,
                startup_timeout_seconds: mount.startup_timeout_seconds,
            })
            .collect(),
    })
}
...
fn from_proto(policy: &SandboxPolicy) -> PolicyFile {
    ...
    PolicyFile {
        ...
        fuse_mounts: policy
            .fuse_mounts
            .iter()
            .map(|mount| FuseMountDef {
                binary: mount.binary.clone(),
                args: mount.args.clone(),
...
pub enum PolicyViolation {
    ...
    /// More than one FUSE mount is declared in the v1 policy.
    TooManyFuseMounts { count: usize },
    /// A FUSE mount declaration is unsafe or malformed.
    InvalidFuseMount { reason: String },
...
impl fmt::Display for PolicyViolation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TooManyFuseMounts { count } => {
                write!(f, "v1 supports at most one FUSE mount, got {count}")
            }
            Self::InvalidFuseMount { reason } => {
                write!(f, "invalid FUSE mount: {reason}")
            }
...
fn validate_fuse_mount(
    mount: &openshell_core::proto::FuseMount,
    violations: &mut Vec<PolicyViolation>,
) {
    let invalid = |reason: String, violations: &mut Vec<PolicyViolation>| {
        violations.push(PolicyViolation::InvalidFuseMount { reason });
    };

    for (field, value) in [
        ("binary", mount.binary.as_str()),
        ("mountpoint", mount.mountpoint.as_str()),
    ] {
        let path = Path::new(value);
        if !path.is_absolute() {
            invalid(format!("{field} must be absolute: {value}"), violations);
        }
        if path
            .components()
        ...
        {
            invalid(
                format!("{field} must not contain '..': {value}"),
                violations,
            );
        }
        if normalize_path(value) != value.trim_end_matches('/') {
            invalid(format!("{field} must be normalized: {value}"), violations);
        }
    }

    if mount.binary.starts_with("/sandbox/") || mount.binary == "/sandbox" {
        invalid(
            "binary must be outside the writable /sandbox tree".into(),
            violations,
        );
    }
    if mount.mountpoint == "/sandbox" || !mount.mountpoint.starts_with("/sandbox/") {
        invalid(
            "mountpoint must be a child of /sandbox, not /sandbox itself".into(),
            violations,
        );
    }
    if mount.binary.is_empty() || mount.mountpoint.is_empty() {
        invalid("binary and mountpoint must be nonempty".into(), violations);
    }
    if mount.fs_name.is_empty()
        || mount.fs_name.len() > 64
        || !mount
            .fs_name
            .bytes()
    ...
    {
        invalid(
            "fs_name must be 1-64 ASCII identifier characters".into(),
            violations,
        );
    }
    if mount.args.len() > 32 || mount.args.iter().map(String::len).sum::<usize>() > 4096 {
        invalid("args exceed the v1 count or byte limit".into(), violations);
    }
    if mount.args.iter().any(|arg| arg.as_bytes().contains(&0)) {
        invalid("args must not contain NUL".into(), violations);
    }
    if mount.startup_timeout_seconds > 120 {
        invalid(
            "startup_timeout_seconds must be at most 120".into(),
            violations,
        );
    }
}
...
pub fn validate_sandbox_policy(
    ...
    if policy.fuse_mounts.len() > 1 {
        violations.push(PolicyViolation::TooManyFuseMounts {
            count: policy.fuse_mounts.len(),
        });
    }
    for mount in &policy.fuse_mounts {
        validate_fuse_mount(mount, &mut violations);
    }
...
mod tests {
    ...
    #[test]
    fn fuse_mount_round_trip_preserves_static_declaration() {
        let yaml = r#"
version: 1
fuse_mounts:
...
"#;
        let policy = parse_sandbox_policy(yaml).expect("FUSE policy should parse");
        assert_eq!(policy.fuse_mounts.len(), 1);
        assert_eq!(policy.fuse_mounts[0].mountpoint, "/sandbox/work");
        validate_sandbox_policy(&policy).expect("FUSE policy should validate");

        let serialized = serialize_sandbox_policy(&policy).expect("FUSE policy should serialize");
        let round_trip = parse_sandbox_policy(&serialized).expect("serialized policy should parse");
        assert_eq!(round_trip.fuse_mounts, policy.fuse_mounts);
    }

    #[test]
    fn fuse_mount_validation_rejects_writable_binary_and_root_mount() {
        let policy = parse_sandbox_policy(
            r#"
version: 1
fuse_mounts:
  - binary: /sandbox/openeral-fused
    mountpoint: /sandbox
    fs_name: openeral
"#,
        )
        ...
        assert!(
            violations
                .iter()
                .filter(|violation| matches!(violation, PolicyViolation::InvalidFuseMount { .. }))
                .count()
                >= 2
        );
    }
