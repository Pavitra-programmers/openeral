...
func SandboxPolicyFromProto(p *sbv1.SandboxPolicy) *types.SandboxPolicy {
	...
	if mounts := p.GetFuseMounts(); mounts != nil {
		result.FuseMounts = make([]types.FuseMount, 0, len(mounts))
		for _, mount := range mounts {
			if mount != nil {
				result.FuseMounts = append(result.FuseMounts, fuseMountFromProto(mount))
			}
		}
	}
...
func SandboxPolicyToProto(p *types.SandboxPolicy) *sbv1.SandboxPolicy {
	...
	if p.FuseMounts != nil {
		result.FuseMounts = make([]*sbv1.FuseMount, 0, len(p.FuseMounts))
		for i := range p.FuseMounts {
			result.FuseMounts = append(result.FuseMounts, fuseMountToProto(&p.FuseMounts[i]))
		}
	}
...
func fuseMountFromProto(m *sbv1.FuseMount) types.FuseMount {
	return types.FuseMount{
		Binary:                m.GetBinary(),
		Args:                  CopyStringSlice(m.GetArgs()),
		Mountpoint:            m.GetMountpoint(),
		FSName:                m.GetFsName(),
		AllowOther:            m.GetAllowOther(),
		StartupTimeoutSeconds: m.GetStartupTimeoutSeconds(),
	}
}

func fuseMountToProto(m *types.FuseMount) *sbv1.FuseMount {
	return &sbv1.FuseMount{
		Binary:                m.Binary,
		Args:                  CopyStringSlice(m.Args),
		Mountpoint:            m.Mountpoint,
		FsName:                m.FSName,
		AllowOther:            m.AllowOther,
		StartupTimeoutSeconds: m.StartupTimeoutSeconds,
	}
}
