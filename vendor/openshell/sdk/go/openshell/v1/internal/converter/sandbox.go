...
func SandboxSpecToProto(spec *types.SandboxSpec) *pb.SandboxSpec {
	...
	if spec.GPUCount != nil || spec.Fuse {
		result.ResourceRequirements = &pb.ResourceRequirements{
			Fuse: nil,
		}
		if spec.GPUCount != nil {
			result.ResourceRequirements.Gpu = &pb.GpuResourceRequirements{Count: spec.GPUCount}
		}
		if spec.Fuse {
			result.ResourceRequirements.Fuse = &pb.FuseResourceRequirements{}
		}
	}
