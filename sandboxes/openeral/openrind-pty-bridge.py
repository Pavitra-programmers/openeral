...
Usage:  openrind-pty-bridge.py [--framed] <command> [args...]
        (e.g. `openrind-pty-bridge.py --framed claude`)
...
# Desktop-selected Claude sessions launch the bridge in forced framed mode.
# This is emitted only AFTER fd 0 has been set raw. The desktop transport
# consumes it internally, so it is never terminal content.
READY_MAGIC = b"\x00OPENRINDPTYREADY1\x00"
...
def main():
    ...
    argv = sys.argv[1:]
    force_framed = bool(argv and argv[0] == "--framed")
    if force_framed:
        argv = argv[1:]
    if not argv:
    ...
    # Desktop connects use --framed. That lets us set the transport tty raw and
    # signal readiness BEFORE the desktop sends a single control byte; otherwise
    # the shell that starts this bridge can echo the handshake as terminal junk.
    inbound = bytearray()
    init_cols = 0
    init_rows = 0
    if force_framed:
