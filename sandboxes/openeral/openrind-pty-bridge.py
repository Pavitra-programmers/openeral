...
Why this exists
---------------
On Windows, Openrind Desktop used to put a local Windows **ConPTY**
(Pseudoconsole) in front of `wsl.exe`. ConPTY keeps its own screen model and
...
def main():
    ...
    while True:
        ...
        # Service keyboard/control input FIRST when both directions are ready.
        # Full-screen TUIs can keep master_fd continuously readable while they
        # redraw. Handling output first and immediately continuing the loop
        # starved stdin until a quiet gap, turning a keystroke or Enter into a
        # multi-second delay. One input read per readiness cycle provides fair,
        # bounded-latency input without reducing output throughput.
        if 0 in readable:
            try:
                chunk = os.read(0, READ_CHUNK)
            except OSError:
        ...
        if master_fd in readable:
            ...
            # Release any bytes held for an escape sequence that never
            # completed, so an idle agent's last output is never stuck.
...
if __name__ == "__main__":
    try:
        ...
        # stderr is intentionally redirected away from the TUI, so an
        # unhandled bridge/bootstrap failure would otherwise look like a blank
        # connected terminal. Preserve the detailed traceback in the private
        # log and send one actionable line through the terminal byte stream.
        log("fatal bridge error: %r" % (exc,))
        try:
            write_all(
                1,
                (
                    "\r\nOpenrind Shell could not allocate the Claude terminal: %s\r\n"
                    % (exc,)
