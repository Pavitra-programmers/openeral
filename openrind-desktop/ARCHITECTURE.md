# Openrind Desktop Architecture

## Design principle: Predictable > Clever

Openrind Desktop optimizes for **predictability** over "clever" auto-detection. Users should be able to form a correct mental model of what will happen.
...
- `OPENRIND_DESKTOP_DOCKER_BIN` (absolute path to `docker`)
...
1. Honor `OPENRIND_DESKTOP_DOCKER_BIN` if set.
...
We move most of the functionality to the openrind-desktop server which interfaces mostly with FS and proxies to opencode.



## Filesystem mutation policy

Openrind Desktop should route filesystem mutations through the Openrind Desktop server whenever possible.
...
- Any UI feature that changes workspace files or config should call an Openrind Desktop server endpoint first.
- Local Tauri filesystem commands are a host-mode fallback, not the primary product surface.
- If a feature cannot yet write through the Openrind Desktop server, treat that as an architecture gap and close it before depending on direct local writes.
- Reads can fall back locally when necessary, but writes should be designed around the Openrind Desktop server path.

## Agent authority map

When Openrind Desktop is edited from `openrind-desktop-enterprise`, architecture and runtime behavior should be sourced from this document.

| Entry point | Role | Architecture authority |
| --- | --- | --- |
| `openrind-desktop-enterprise/AGENTS.md` | Openrind Desktop Factory multi-repo orchestration | Defers Openrind Desktop runtime flow, server-vs-shell ownership, and filesystem mutation behavior to `_repos/openrind-desktop/ARCHITECTURE.md`. |
| `openrind-desktop-enterprise/.opencode/agents/openrind-desktop-surgeon.md` | Surgical fix agent for `_repos/openrind-desktop` | Uses `_repos/openrind-desktop/ARCHITECTURE.md` as the runtime and architecture source of truth before changing product behavior. |
| `_repos/openrind-desktop/AGENTS.md` | Product vocabulary, audience, and repo-local development guidance | Refers to `ARCHITECTURE.md` for runtime flow, server ownership, and architectural boundaries. |
| Skills / commands / agents that mutate workspace state | Capability layer on top of the product runtime | Should assume the Openrind Desktop server path is canonical for workspace creation, config writes, `.opencode/` mutation, and reload signaling. |

### Agent access to server-owned behavior

Agents, skills, and commands should model the following as Openrind Desktop server behavior first:

- workspace creation and initialization
- writes to `.opencode/`, `opencode.json`, and `opencode.jsonc`
- Openrind Desktop workspace config writes (`.opencode/openrind-desktop.json`)
- share-bundle publish/fetch flows for supported Openrind Desktop capability bundles such as skills
...
Openrind Desktop uses a single reload-required flow for changes that only take effect when OpenCode restarts.
...
When the desktop shell asks the Openrind Desktop server to manage OpenCode, the managed
...
through the Openrind Desktop server and `x-opencode-directory`, but startup no longer
...
## Core Concepts of Openrind Desktop
...
- adds a new abstraction "workspace" is a project folder and a simple .json file that includes a list of opencode primitives that map perfectly to an opencode workdir (not fully implemented)
  - openrind-desktop can open a workpace.json and decide where to populate a folder with thse settings (not implemented today

## Repository/component map

- `/apps/app/`: Openrind Desktop app UI (desktop/mobile/web client experience layer).
- `/apps/desktop/`: Tauri desktop shell that hosts the app UI and manages native process lifecycles.
- `/apps/server/`: Openrind Desktop server (API/control layer consumed by the app).
- `/apps/orchestrator/`: Openrind Desktop orchestrator CLI/daemon. In `start`/`serve` host mode it manages Openrind Desktop server + OpenCode; in daemon mode it manages worker/sandbox lifecycle.
- `/apps/share/`: share-link publisher service for Openrind Desktop bundle imports.
- `/ee/apps/landing/`: Openrind Desktop landing page surfaces.
...
Openrind Desktop is a client experience that consumes Openrind Desktop server surfaces.

Openrind Desktop supports two product runtime modes for users:

- desktop
- web/cloud (also usable from mobile clients)

Openrind Desktop therefore has two runtime connection modes:

### Mode A - Desktop

- Openrind Desktop runs on a desktop/laptop and can host Openrind Desktop server surfaces locally.
- The OpenCode server runs on loopback (default `127.0.0.1:4096`).
- The Openrind Desktop server also defaults to loopback-only access. Remote sharing is an explicit opt-in that rebinds the Openrind Desktop server to `0.0.0.0` while keeping OpenCode on loopback.
- Openrind Desktop UI connects via the official SDK and listens to events.
- Openrind Desktop server is the local API/control layer for this mode and owns the managed OpenCode child lifecycle.

### Mode B - Web/Cloud (can be mobile)

- User signs in to hosted Openrind Desktop web/app surfaces (including mobile browser/client access).
- User launches a cloud worker from hosted control plane.
- Openrind Desktop returns remote connect credentials (`/w/ws_*` URL + access token).
- User connects from Openrind Desktop app using `Add a worker` -> `Connect remote`.
...
- `/apps/desktop/` exposes native commands (`engine_*`, `orchestrator_*`, `openrind_desktop_server_*`) to start/stop local services and report status to the UI.
- `/apps/desktop/` is also the source of truth for desktop bootstrap config that must survive updates, including Den server targeting and forced-sign-in startup behavior. The shell reads a predictable external `desktop-bootstrap.json` from the host config directory (or `OPENRIND_DESKTOP_DESKTOP_BOOTSTRAP_PATH` when explicitly overridden). Default builds consume that file when present; custom builds seed or overwrite it when their bundled bootstrap differs from the standard default.
- Desktop host runtime is server-managed: the shell starts Openrind Desktop server with managed OpenCode enabled, and the UI consumes Openrind Desktop server APIs.
- Openrind Desktop server (`/apps/server/`) is the API surface consumed by the UI; it proxies OpenCode routes for the active workspace.
- Desktop-launched OpenCode credentials are always random, per-launch values generated by Openrind Desktop. OpenCode stays on loopback and is intended to be reached through Openrind Desktop server rather than exposed directly.
...
/apps/desktop (Tauri shell)
    |
    +--> /apps/server (Openrind Desktop API + proxy surface)
               |
               +--> OpenCode
```
...
- `/ee/apps/den-worker-runtime/` defines the runtime packaging and boot path used inside cloud workers (including Docker/snapshot artifacts and `openrind-desktop serve` startup assumptions).
...
- The Openrind Desktop app (desktop or mobile client) connects to worker Openrind Desktop server surfaces via URL + token (`/w/ws_*` when available).
...
/ee/apps/den-api (formerly /ee/apps/den-controller)
    ...
    |        |
    |        v
    |      /ee/apps/den-worker-runtime -> openrind-desktop serve + OpenCode
    |
    +--> /ee/apps/den-worker-proxy (signed preview + proxy)

Openrind Desktop app/mobile client
    -> Connect remote (URL + token)
    -> worker Openrind Desktop server surface
```

## Messaging Bridge

Openrind Desktop no longer starts or proxies an app-owned local messaging bridge in the desktop host runtime. Messaging surfaces must be provided by an external server/worker surface rather than Tauri, Electron, or Openrind Desktop server launching a local `opencode-router` child.
...
Desktop local Openrind Desktop server ports:

- Desktop-hosted local Openrind Desktop server instances do not assume a fixed `8787` port.
...
1. Authenticate in Openrind Desktop Cloud control surface.
...
5. Connect in Openrind Desktop app via deep link or manual URL + token.
...
In Openrind Desktop, the long-term direction is:

- Use the Openrind Desktop server (`/apps/server/`) as the single API surface for filesystem-backed operations.
...
Openrind Desktop uses the official JavaScript/TypeScript SDK:
...
Openrind Desktop must be real-time. It subscribes to SSE events:
...
Openrind Desktop maps a "Task Run" to an OpenCode **Session**.
...
Openrind Desktop's file browser and "what changed" UI are powered by:
...
Openrind Desktop must surface permission requests clearly and respond explicitly.

- Permission response API:
  - `client.permission.reply({ requestID, reply })` (where `reply` is `once` | `always` | `reject`)

Openrind Desktop UI should:
...
Openrind Desktop's settings pages use:
...
Openrind Desktop exposes two extension surfaces:
...
2. **Plugins (OpenCode)**
   ...
   - Openrind Desktop should show plugin status and instructions; a native plugin manager is planned.

### Engine reload (config refresh)

- Openrind Desktop server exposes `POST /workspace/:id/engine/reload`.
...
- Reload requests follow Openrind Desktop server approval rules.

### Skill Registry (Current + Future)

- Today, Openrind Desktop only supports **curated lists + manual sources**.
- Future goals:
...
Openrind Desktop conceptually treats "workspace" as the current project/path.

## Optional TUI Control (Advanced)

The SDK exposes `client.tui.*` methods. Openrind Desktop can optionally provide a "Developer Mode" screen to:
...
Openrind Desktop enforces folder access through **two layers**:

1. **Openrind Desktop UI authorization**
   - user explicitly selects allowed folders via native picker
   - Openrind Desktop remembers allowed roots per profile

2. **OpenCode server permissions**
   - OpenCode requests permissions as needed
   - Openrind Desktop intercepts requests via events and displays them
