...
| ---- | ---- | ------------------------ |
| 01   | Ready to ship the migration | Tauri users see "Openrind Desktop is moving" prompt on next update |
...
- Pushes to `dev` or `main` refresh the rolling prerelease bucket at
  <https://github.com/openrind/openrind-desktop/releases/tag/electron-preview-latest>.
...
- The install handoff requires a pasted Electron artifact URL plus two explicit
  ...
  `Openrind Desktop.app.migrate-bak` for rollback.
...
3. Ask testers to download the matching platform artifact:
   ...
   - Windows: `openrind-desktop-win-x64-*.exe`
   - Linux: `openrind-desktop-linux-x64-*.AppImage` or `.tar.gz`
...
node scripts/migration/01-cut-migration-release.mjs --version 0.12.0 \
  --mac-url 'https://github.com/openrind/openrind-desktop/releases/download/v0.12.0/Openrind Desktop-darwin-arm64-0.12.0-mac.zip' \
...
3. Creates a release-config fragment at
   ...
   `VITE_OPENRIND_DESKTOP_MIGRATION_RELEASE=1` and the per-platform download
...
- Users still on Tauri: the migrate prompt is gated on
  `VITE_OPENRIND_DESKTOP_MIGRATION_RELEASE=1` at build time. Re-cut the
...
- Users mid-migration: the Rust `migrate_to_electron` command keeps
  the previous `.app` at `Openrind Desktop.app.migrate-bak`. Instruct users
  to `mv Openrind Desktop.app.migrate-bak Openrind Desktop.app` if the Electron
  launch fails.
