...
async function main() {
  ...
  const workDir = join(tmpdir(), `openrind-desktop-migrate-validate-${args.tag}`);
  ...
  const appPath = join(unzipDir, "Openrind Desktop.app");
  if (!existsSync(appPath)) {
    console.log(`[validate] ✗ Openrind Desktop.app not found inside zip (got: ${unzipDir})`);
  ...
  const steps = [
    ...
    "Restart, see the 'Openrind Desktop is moving' modal, click 'Install now'.",
