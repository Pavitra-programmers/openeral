...
function sanitizeOpenrindDesktopTemplateConfig(openrindDesktop: Record<string, unknown> | null | undefined) {
  const next = cloneJson(openrindDesktop ?? {});
...
function normalizeBundleFetchUrl(bundleUrl: unknown) {
  ...
  if (inputUrl.origin !== trustedBaseUrl.origin) {
    throw new RouteError(400, "invalid_request", `Shared bundle URLs must use the configured Openrind Desktop publisher (${trustedBaseUrl.origin}).`);
  }
...
function resolvePublisherBaseUrl() {
  return String(process.env.OPENRIND_DESKTOP_PUBLISHER_BASE_URL ?? "").trim() || "https://share.openrind-desktoplabs.com";
}

function resolvePublisherOrigin() {
  return String(process.env.OPENRIND_DESKTOP_PUBLISHER_REQUEST_ORIGIN ?? "").trim() || "https://app.openrind-desktop.software";
}
...
export function createManagedResourceService(input: {
  ...
  function workspaceSkillPath(workspace: WorkspaceRecord, key: string) {
    ...
    return path.join(baseDir, ".opencode", "skills", "openrind-desktop-managed", key, "SKILL.md");
  }
  ...
  function upsertManaged(kind: ManagedKind, payload: {
    ...
    const item = kindConfig[kind].itemRepo.upsert({
      ...
      source: payload.source ?? "openrind_desktop_managed",
    });
  ...
  return {
    ...
    async upsertWorkspaceSkill(workspaceId: string, payload: {
      ...
      const item = upsertManaged("skills", {
        ...
        source: payload.source ?? existing?.source ?? "openrind_desktop_managed",
        workspaceIds: [workspace.id],
      });
    ...
    async listHubSkills(repo?: Partial<HubRepo>) {
      ...
      const listing = await fetch(`https://api.github.com/repos/${encodeURIComponent(resolvedRepo.owner)}/${encodeURIComponent(resolvedRepo.repo)}/contents/skills?ref=${encodeURIComponent(resolvedRepo.ref)}`, {
        headers: { Accept: "application/vnd.github+json", "User-Agent": "openrind-desktop-server-v2" },
      });
      if (!listing.ok) {
      ...
      for (const entry of Array.isArray(items) ? items : []) {
        ...
        try {
          const content = await fetch(`${rawBase}/skills/${encodeURIComponent(name)}/SKILL.md`, {
            headers: { Accept: "text/plain", "User-Agent": "openrind-desktop-server-v2" },
    ...
    async exportWorkspace(workspaceId: string, options?: { sensitiveMode?: WorkspaceExportSensitiveMode }) {
      ...
      const openrindDesktop = sanitizeOpenrindDesktopTemplateConfig(snapshot.stored.openrindDesktop);
    ...
    async importWorkspace(workspaceId: string, payload: Record<string, unknown>) {
      ...
      const openrindDesktop = readRecord(payload.openrindDesktop);
      ...
      if (openrindDesktop) {
          await input.config.patchWorkspaceConfig(workspaceId, {
            openrindDesktop: sanitizeOpenrindDesktopTemplateConfig(openrindDesktop),
          });
      }
      ...
      try {
        const response = await fetch(`${resolvePublisherBaseUrl().replace(/\/+$/, "")}/v1/bundles`, {
          ...
          headers: {
            ...
            ...(normalizeString(inputValue.name) ? { "X-OpenrindDesktop-Name": normalizeString(inputValue.name) } : {}),
            "X-OpenrindDesktop-Bundle-Type": bundleType,
