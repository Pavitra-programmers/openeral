...
function readArgs(argv: string[]): BuildOptions {
  const options: BuildOptions = {
    bundleDir: process.env.OPENRIND_DESKTOP_SERVER_V2_BUNDLE_DIR?.trim() ? resolve(process.env.OPENRIND_DESKTOP_SERVER_V2_BUNDLE_DIR.trim()) : null,
    embedRuntime: false,
    filename: "openrind-desktop-server-v2",
...
function createEmbeddedEntrypoint(assets: RuntimeAssetPaths) {
  const buildDir = mkdtempSync(join(os.tmpdir(), "openrind-desktop-server-v2-build-"));
...
function buildOnce(options: BuildOptions, target?: string) {
  ...
  const args = [
    ...
    outfile,
    "--define",
    `__OPENRIND_DESKTOP_SERVER_V2_VERSION__=${JSON.stringify(version)}`,
  ];
  if (target) {
