...
const explicitStartUrl = process.env.OPENRIND_DESKTOP_ELECTRON_START_URL?.trim() || "";
...
// OPENRIND_DESKTOP_ELECTRON_REMOTE_DEBUG_PORT=<port> or set to "0" to disable.
...
electronChild = run(pnpmCmd, [...pnpmArgs, "exec", "electron", "./electron/main.mjs"], {
  ...
  env: {
    ...process.env,
    OPENRIND_DESKTOP_DEV_MODE: process.env.OPENRIND_DESKTOP_DEV_MODE ?? "1",
    OPENRIND_DESKTOP_ELECTRON_START_URL: resolvedStartUrl,
    ...(cdpPort ? { OPENRIND_DESKTOP_ELECTRON_REMOTE_DEBUG_PORT: cdpPort } : {}),
  },
});

if (cdpPort) {
  console.log(`[openrind-desktop] Electron CDP exposed at http://127.0.0.1:${cdpPort}`);
}
