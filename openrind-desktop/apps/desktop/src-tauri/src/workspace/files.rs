...
use crate::types::{OpencodeCommand, WorkspaceOpenrindDesktopConfig};
...
description: Workspace guide to introduce OpenrindDesktop and onboard new users.
---

# Welcome to OpenrindDesktop

Hi, I'm Ben and this is OpenrindDesktop. It's an open-source alternative to Claude's cowork. It helps you work on your files with AI and automate the mundane tasks so you don't have to.
...
OpenrindDesktop feels like a chat app, but it can safely work with the files you allow. Put files in this workspace and I can summarize them, create new ones, or help organize them.
...
OpenrindDesktop is a GUI for OpenCode. Everything that works in OpenCode works here.

Most reliable setup today:
1) Install the OpenrindDesktop-pinned OpenCode version
2) Configure providers there (models and API keys)
3) Come back to OpenrindDesktop and start a session
...
End with two friendly next actions to try in OpenrindDesktop."#;
...
- Reply with these four lines, exactly and in order:
  1) hey there welcome this is openrind-desktop
...
fn seed_openrind_desktop_agent(agent_root: &PathBuf) -> Result<(), String> {
    let agent_path = agent_root.join("openrind-desktop.md");
    ...
    let doc = r#"---
description: OpenrindDesktop default agent (safe, mobile-first, self-referential)
mode: primary
temperature: 0.2
---

You are OpenrindDesktop.

When the user refers to \"you\", they mean the OpenrindDesktop app and the current workspace.
...
pub fn ensure_workspace_files(workspace_path: &str, preset: &str) -> Result<(), String> {
    ...
    seed_openrind_desktop_agent(&agents_dir)?;
    ...
    if let Some(obj) = config.as_object_mut() {
        ...
        if current.is_empty() {
            obj.insert(
                ...
                serde_json::Value::String("openrind-desktop".to_string()),
            );
            config_changed = true;
        }
    }
    ...
    if !openrind_desktop_path.exists() {
        ...
        fs::write(
            &openrind_desktop_path,
            serde_json::to_string_pretty(&openrind_desktop).map_err(|e| e.to_string())?,
        )
        .map_err(|e| format!("Failed to write {}: {e}", openrind_desktop_path.display()))?;
    }
