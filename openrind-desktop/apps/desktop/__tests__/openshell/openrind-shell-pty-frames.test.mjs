...
// buffering until bridge readiness, UTF-8 stream decoding, and exit propagation.
...
test("makePipePty: does not send in-band bytes before the bridge is ready", () => {
  const child = makeFakeChild();
  makePipePty(child, 100, 40);
  assert.equal(child.stdin.writes.length, 0);
});

test("makePipePty: waits for READY_MAGIC, hides it, then flushes frames in order", () => {
  const child = makeFakeChild();
  const term = makePipePty(child, 80, 24);
  const output = [];
  term.onData((text) => output.push(text));

  term.resize(90, 30); // queued (not ready yet)
  term.write("x"); // queued
  assert.equal(child.stdin.writes.length, 0, "frames must not write before readiness");

  // The marker may be split across stdout chunks; it is internal control data.
  child.stdout.emit("data", READY_MAGIC.subarray(0, 5));
  assert.equal(child.stdin.writes.length, 0);
  child.stdout.emit("data", Buffer.concat([READY_MAGIC.subarray(5), Buffer.from("banner")]));

  // Initial resize plus queued frames flush only once raw mode is proven live.
  ...
  assert.equal(output.join(""), "banner", "the readiness marker must not reach xterm");
