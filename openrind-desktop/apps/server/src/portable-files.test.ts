...
async function makeWorkspace(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "openrind-desktop-portable-files-"));
  tempDirs.push(dir);
...
describe("portable files", () => {
  test("lists only extra shareable .opencode files", async () => {
    ...
    await writeFile(join(workspaceRoot, ".opencode", "agents", "openrind-desktop.md"), "# agent\n", "utf8");
    ...
    await writeFile(join(workspaceRoot, ".opencode", "openrind-desktop.json"), '{"version":1}\n', "utf8");
    ...
    expect(files).toEqual([
      { path: ".opencode/agents/openrind-desktop.md", content: "# agent\n" },
  ...
  test("rejects non-allowlisted portable files and path traversal", async () => {
    ...
    expect(() =>
      planPortableFiles(workspaceRoot, [{ path: ".opencode/openrind-desktop.json", content: "{}" }]),
    ).toThrow(/not allowed/i);
