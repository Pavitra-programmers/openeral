...
async function createWorkspaceRoot() {
  const root = await mkdtemp(join(tmpdir(), "openrind-desktop-session-read-"));
...
function startOpenrindDesktopServer(input: { workspaceRoot: string; opencodeBaseUrl: string }) {
...
describe("workspace session read APIs", () => {
  test("lists sessions and returns session details, messages, and snapshot", async () => {
    ...
    const openrindDesktop = startOpenrindDesktopServer({
      workspaceRoot,
