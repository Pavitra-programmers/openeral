...
import { opencodeConfigPath, openrindDesktopConfigPath, projectCommandsDir, projectSkillsDir } from "./workspace-files.js";
...
export function createServerLogger(config: ServerConfig): ServerLogger {
  const runId = process.env.OPENRIND_DESKTOP_RUN_ID ?? shortId();
    ...
    fetch: async (request: Request) => {
      ...
      try {
        ...
        if (!(error instanceof ApiError)) {
          console.error("[openrind-desktop-server] Unhandled error:", error);
        }
        const apiError = error instanceof ApiError
          ? error
          : new ApiError(500, "internal_error", "Unexpected server error");
...
async function proxyOpencodeRequest(input: {
  ...
  headers.delete("x-openrinddesktop-host-token");
  headers.delete("x-openrinddesktop-client-id");
  headers.delete("host");
...
function withCors(response: Response, request: Request, config: ServerConfig) {
  ...
  headers.set(
    ...
    "Authorization, Content-Type, X-OpenrindDesktop-Host-Token, X-OpenrindDesktop-Client-Id, X-OpenCode-Directory, X-Opencode-Directory, x-opencode-directory",
  );
...
async function requireClient(request: Request, config: ServerConfig, tokens: TokenService): Promise<Actor> {
  ...
  const clientId = request.headers.get("x-openrinddesktop-client-id") ?? undefined;
...
function requireHostToken(request: Request, config: ServerConfig): Actor {
  const hostToken = request.headers.get("x-openrinddesktop-host-token");
...
function buildCapabilities(config: ServerConfig): Capabilities {
  ...
  return {
    ...
    skills: { read: true, write: writeEnabled, source: "openrind-desktop" },
    hub: {
      skills: {
        read: true,
    ...
    toolProviders: {
      ...
      files: {
        ...
        inboxPath: ".opencode/openrind-desktop/inbox/",
        outboxPath: ".opencode/openrind-desktop/outbox/",
        maxBytes,
      },
    },
  };
...
function resolveSandboxBackend(): Capabilities["sandbox"]["backend"] {
  const raw = (process.env.OPENRIND_DESKTOP_SANDBOX_BACKEND ?? "").trim().toLowerCase();
...
function resolveSandboxEnabled(backend: Capabilities["sandbox"]["backend"]): boolean {
  const raw = (process.env.OPENRIND_DESKTOP_SANDBOX_ENABLED ?? "").trim().toLowerCase();
...
function resolveInboxEnabled(): boolean {
  const raw = (process.env.OPENRIND_DESKTOP_INBOX_ENABLED ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

function resolveOutboxEnabled(): boolean {
  const raw = (process.env.OPENRIND_DESKTOP_OUTBOX_ENABLED ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

function resolveInboxMaxBytes(): number {
  const raw = (process.env.OPENRIND_DESKTOP_INBOX_MAX_BYTES ?? "").trim();
...
function resolveToyUiEnabled(): boolean {
  const raw = (process.env.OPENRIND_DESKTOP_TOY_UI ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

// Dev-only log sink target. When OPENRIND_DESKTOP_DEV_LOG_FILE is set to a path, the
...
function resolveDevLogPath(): string | null {
  const raw = (process.env.OPENRIND_DESKTOP_DEV_LOG_FILE ?? "").trim();
  return raw.length > 0 ? raw : null;
}

function resolveBrowserProvider(): Capabilities["toolProviders"]["browser"] {
  const raw = (process.env.OPENRIND_DESKTOP_BROWSER_PROVIDER ?? "").trim().toLowerCase();
...
function resolveInboxDir(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "openrind-desktop", "inbox");
}

function resolveOutboxDir(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "openrind-desktop", "outbox");
}
...
function createRoutes(
  ...
  // tokens; it is also a no-op when OPENRIND_DESKTOP_DEV_LOG_FILE is unset.
  ...
  addRoute(routes, "GET", "/ui/assets/openrind-desktop-mark.svg", "none", async () => {
    if (!resolveToyUiEnabled()) {
      throw new ApiError(404, "ui_disabled", "Toy UI is disabled");
    }
      ...
      if (error instanceof InvalidEnvKeyError) {
        throw new ApiError(
          ...
          error.code === "reserved_env_key"
            ? "Environment variable name is reserved for Openrind Desktop internals"
  ...
  addRoute(routes, "DELETE", "/workspaces/:id", "host", async (ctx) => {
    ...
    await recordAudit(workspace.path, {
      ...
      target: "workspace",
      summary: "Deleted workspace from Openrind Desktop server",
      timestamp: Date.now(),
    });
  ...
  addRoute(routes, "GET", "/workspace/:id/config", "client", async (ctx) => {
    ...
    const openrindDesktop = await readOpenrindDesktopConfig(workspace.path);
    const lastAudit = await readLastAudit(workspace.path, workspace.id);
    return jsonResponse({ opencode, openrindDesktop, updatedAt: lastAudit?.timestamp ?? null });
  });
  ...
  addRoute(routes, "PATCH", "/workspace/:id/config", "client", async (ctx) => {
    ...
    const openrindDesktop = body.openrindDesktop as Record<string, unknown> | undefined;

    if (!opencode && !openrindDesktop) {
      throw new ApiError(400, "invalid_payload", "opencode or openrind-desktop updates required");
    }
    ...
    await requireApproval(ctx, {
      ...
      paths: [opencode ? opencodeConfigPath(workspace.path) : null, openrindDesktop ? openrindDesktopConfigPath(workspace.path) : null].filter(Boolean) as string[],
    });
    ...
    if (openrindDesktop) {
      await writeOpenrindDesktopConfig(workspace.path, openrindDesktop, true);
    }
  ...
  addRoute(routes, "POST", "/workspace/:id/import", "client", async (ctx) => {
    ...
    await requireApproval(ctx, {
      ...
      paths: [
        ...
        openrindDesktopConfigPath(workspace.path),
  ...
  addRoute(routes, "POST", "/share/bundles/publish", "client", async (ctx) => {
    ...
    if (typeof body.baseUrl === "string" && body.baseUrl.trim()) {
      throw new ApiError(
        ...
        "Bundle publishing always uses the configured Openrind Desktop publisher. Remove baseUrl from the request.",
      );
    }
...
type OpenrindDesktopServerConfigFile = Record<string, unknown> & {
  workspaces?: Array<Record<string, unknown>>;
  authorizedRoots?: string[];
};

async function readServerConfigFile(configPath: string): Promise<OpenrindDesktopServerConfigFile> {
  ...
  try {
    ...
    return ensurePlainObject(JSON.parse(raw)) as OpenrindDesktopServerConfigFile;
  } catch (error) {
...
function serializeWorkspaceConfigEntry(workspace: WorkspaceInfo): Record<string, unknown> {
  return {
    ...
    ...(workspace.openrindDesktopHostUrl ? { openrindDesktopHostUrl: workspace.openrindDesktopHostUrl } : {}),
    ...(workspace.openrindDesktopToken ? { openrindDesktopToken: workspace.openrindDesktopToken } : {}),
    ...(workspace.openrindDesktopWorkspaceId ? { openrindDesktopWorkspaceId: workspace.openrindDesktopWorkspaceId } : {}),
    ...(workspace.openrindDesktopWorkspaceName ? { openrindDesktopWorkspaceName: workspace.openrindDesktopWorkspaceName } : {}),
...
async function persistServerWorkspaceState(config: ServerConfig): Promise<boolean> {
  ...
  const next: OpenrindDesktopServerConfigFile = {
    ...parsed,
...
function getRuntimeControlConfig(): { baseUrl: string; token: string } | null {
  const baseUrl = process.env.OPENRIND_DESKTOP_CONTROL_BASE_URL?.trim() ?? "";
  const token = process.env.OPENRIND_DESKTOP_CONTROL_TOKEN?.trim() ?? "";
...
async function readOpenrindDesktopConfig(workspaceRoot: string): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    throw new ApiError(422, "invalid_json", "Failed to parse openrind-desktop.json");
  }
}
...
async function writeOpenrindDesktopConfig(workspaceRoot: string, payload: Record<string, unknown>, merge: boolean): Promise<void> {
  const path = openrindDesktopConfigPath(workspaceRoot);
  const next = merge ? { ...(await readOpenrindDesktopConfig(workspaceRoot)), ...payload } : payload;
...
async function exportWorkspace(
  ...
  const openrindDesktop = sanitizeOpenrindDesktopTemplateConfig(await readOpenrindDesktopConfig(workspace.path));
...
async function importWorkspace(workspace: WorkspaceInfo, payload: Record<string, unknown>): Promise<void> {
  ...
  const openrindDesktop = payload.openrindDesktop as Record<string, unknown> | undefined;
  ...
  if (openrindDesktop) {
    const sanitizedOpenrindDesktop = sanitizeOpenrindDesktopTemplateConfig(openrindDesktop);
    if (modes.openrindDesktop === "replace") {
      await writeOpenrindDesktopConfig(workspace.path, sanitizedOpenrindDesktop, false);
    } else {
      await writeOpenrindDesktopConfig(workspace.path, sanitizedOpenrindDesktop, true);
    }
  }
...
async function materializeBlueprintSessions(config: ServerConfig, workspace: WorkspaceInfo): Promise<{
  ...
  const openrindDesktop = await readOpenrindDesktopConfig(workspace.path);
  const templates = normalizeBlueprintSessionTemplates(openrindDesktop);
  if (!templates.length) {
    return { ok: true, created: [], existing: [], openSessionId: null };
  }

  const existing = readMaterializedBlueprintSessions(openrindDesktop);
  ...
  const now = Date.now();
  const nextOpenrindDesktop = applyMaterializedBlueprintSessions(
    openrindDesktop,
    created.map(({ templateId, sessionId }) => ({ templateId, sessionId })),
    now,
  );
  await writeOpenrindDesktopConfig(workspace.path, nextOpenrindDesktop, false);
