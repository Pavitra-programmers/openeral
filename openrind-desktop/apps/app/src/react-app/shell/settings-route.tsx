...
import { createOpenrindDesktopServerStore, useOpenrindDesktopServerStoreSnapshot } from "../domains/connections/openrind-desktop-server-store";
...
export function SettingsRoute() {
  ...
  const [openrindDesktopClient, setOpenrindDesktopClient] = useState<OpenrindDesktopServerClient | null>(null);
    ...
    () =>
      selectedWorkspace
        ? {
            ...
            openrindDesktopWorkspaceName: selectedWorkspace.openrindDesktopWorkspaceName,
          }
  ...
  const reloadWorkspaceEngineFromUi = useCallback(async () => {
    ...
    if (!openrindDesktopClient || !workspaceId) {
      setRouteError(t("app.error_connect_first"));
      return false;
    }

    await openrindDesktopClient.reloadEngine(workspaceId);
    ...
    return true;
  }, [openrindDesktopClient, selectedWorkspaceId]);

  useEffect(() => {
    return reloadCoordinator.registerWorkspaceReloadControls({
      canReloadWorkspaceEngine: () => Boolean(openrindDesktopClient && (selectedWorkspace?.id || selectedWorkspaceId)),
  ...
  const openrindDesktopServerStore = useMemo(
    () =>
      createOpenrindDesktopServerStore({
        startupPreference: () => {
          ...
          const stored = readOpenrindDesktopServerSettings();
          const storedUrl = stored.urlOverride?.trim() ?? "";
          return storedUrl && !isLoopbackOpenrindDesktopServerUrl(storedUrl) ? "server" : "local";
        },
  ...
  const openrindDesktopServerSnapshot = useOpenrindDesktopServerStoreSnapshot(openrindDesktopServerStore);
  ...
  const debugViewProps = useDebugViewModel({
    developerMode,
    openrindDesktopServerStore,
    openrindDesktopServerSnapshot,
    runtimeWorkspaceId: selectedWorkspace?.id ?? null,
    selectedWorkspaceRoot,
    setRouteError,
  });
  ...
  const refreshRouteState = useMemo(() => async () => {
    ...
    try {
      ...
      const { normalizedBaseUrl, resolvedToken, resolvedHostToken } = await resolveOpenrindDesktopConnection();

      if (!normalizedBaseUrl || !resolvedToken) {
        setOpenrindDesktopClient(null);
        setBaseUrl("");
        setToken("");
      ...
      const client = createOpenrindDesktopServerClient({
      ...
      setOpenrindDesktopClient(client);
  ...
  }, [loading, openrindDesktopClient, selectedWorkspace, workspaces]);
  ...
  useEffect(() => {
    openrindDesktopServerStore.start();
    ...
    return () => {
      ...
      openrindDesktopServerStore.dispose();
    };
  ...
  const routeOpenrindDesktopStatus = openrindDesktopClient ? "connected" : "disconnected";
  const routeOpenrindDesktopCapabilities: OpenrindDesktopServerCapabilities | null = openrindDesktopClient
    ? {
        skills: { read: true, write: true, source: "openrind-desktop" },
    ...
    : null;
  const environmentRuntimeKey = buildOpenrindDesktopEnvRuntimeKey({
    baseUrl: openrindDesktopServerSnapshot.openrindDesktopServerBaseUrl || openrindDesktopServerSnapshot.openrindDesktopServerUrl,
    pid: openrindDesktopServerSnapshot.openrindDesktopServerHostInfo?.pid ?? null,
    port: openrindDesktopServerSnapshot.openrindDesktopServerHostInfo?.port ?? null,
  });
  ...
  const handleApplyEnvironmentChanges = async () => {
    ...
    await engineStart(selectedWorkspaceRoot, {
      ...
      runtime: "direct",
      workspacePaths,
      openrindDesktopRemoteAccess: openrindDesktopServerSnapshot.openrindDesktopServerSettings.remoteAccessEnabled === true,
    });
    const reconnected = await openrindDesktopServerStore.reconnectOpenrindDesktopServer();
    if (!reconnected) {
        ...
        return (
          <GeneralSettingsView
            authorizedFoldersPanel={{
              openrindDesktopServerClient: openrindDesktopClient,
              openrindDesktopServerStatus: routeOpenrindDesktopStatus,
              openrindDesktopServerCapabilities: routeOpenrindDesktopCapabilities,
              runtimeWorkspaceId: selectedWorkspace?.id ?? null,
              selectedWorkspaceRoot,
            ...
            onReportIssue={() => platform.openLink("https://github.com/openrind/openrind-desktop/issues/new?template=bug.yml")}
          />
        );
      case "skills":
        return (
          ...
          <ExtensionsView
            ...
            mcpView={
              <McpView
                ...
                setMcpEnabled={
                  routeOpenrindDesktopStatus === "connected" && routeOpenrindDesktopCapabilities?.mcp?.write
      ...
      case "advanced":
        return (
          <AdvancedView
            ...
            headerStatus={openrindDesktopServerSnapshot.openrindDesktopServerStatus}
            clientConnected={Boolean(opencodeClient)}
            opencodeConnectStatus={null}
            openrindDesktopServerStatus={openrindDesktopServerSnapshot.openrindDesktopServerStatus}
            ...
            configView={{
              ...
              openrindDesktopServerStatus: openrindDesktopServerSnapshot.openrindDesktopServerStatus,
              openrindDesktopServerUrl: openrindDesktopServerSnapshot.openrindDesktopServerUrl,
              openrindDesktopServerSettings: openrindDesktopServerSnapshot.openrindDesktopServerSettings,
              openrindDesktopServerHostInfo: openrindDesktopServerSnapshot.openrindDesktopServerHostInfo,
              runtimeWorkspaceId: selectedWorkspace?.id ?? null,
              updateOpenrindDesktopServerSettings: openrindDesktopServerStore.updateOpenrindDesktopServerSettings,
              resetOpenrindDesktopServerSettings: openrindDesktopServerStore.resetOpenrindDesktopServerSettings,
              testOpenrindDesktopServerConnection: openrindDesktopServerStore.testOpenrindDesktopServerConnection,
