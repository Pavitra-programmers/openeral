 ...
 * `VITE_OPENRIND_DESKTOP_MIGRATION_RELEASE=1` or the dev override
 * `VITE_OPENRIND_DESKTOP_FORCE_MIGRATION_PROMPT=1` is set, so landing this code
...
function readBuildTimeConfig(): MigrationConfig | null {
  ...
  const isMigrationRelease = env.VITE_OPENRIND_DESKTOP_MIGRATION_RELEASE === "1";
  // Dev override: set VITE_OPENRIND_DESKTOP_FORCE_MIGRATION_PROMPT=1 in .env.local
  ...
  return {
    macUrl: env.VITE_OPENRIND_DESKTOP_MIGRATION_MAC_URL ?? (isDevForced ? "https://github.com/openrind/openrind-desktop/releases/latest" : undefined),
    ...
    windowsUrl: env.VITE_OPENRIND_DESKTOP_MIGRATION_WINDOWS_URL ?? (isDevForced ? "https://github.com/openrind/openrind-desktop/releases/latest" : undefined),
    windowsX64Url: env.VITE_OPENRIND_DESKTOP_MIGRATION_WINDOWS_X64_URL,
    linuxUrl: env.VITE_OPENRIND_DESKTOP_MIGRATION_LINUX_URL ?? (isDevForced ? "https://github.com/openrind/openrind-desktop/releases/latest" : undefined),
    ...
    >
      <div className="w-[min(440px,90vw)] rounded-[24px] border border-dls-border bg-dls-surface p-6 shadow-xl">
        ...
        <p className="mt-3 text-sm leading-relaxed text-gray-11">
          ...
          are. You'll be back in Openrind Desktop automatically when it's done.
        </p>
        {error ? (
