...
import type { OpenrindDesktopHubRepo, OpenrindDesktopServerClient } from "../../../../app/lib/openrind-desktop-server";
import {
  createDenClient,
...
import type { OpenrindDesktopServerStore } from "../../connections/openrind-desktop-server-store";
...
const HUB_REPOS_STORAGE_KEY = "openrind-desktop.skills.hubRepos.v1";
...
export function createExtensionsStore(options: {
  ...
  let disposed = false;
  let started = false;
  let stopOpenrindDesktopSubscription: (() => void) | null = null;
  ...
  const readWorkspaceOpenrindDesktopConfigRecord = async (): Promise<Record<string, unknown>> => {
    const root = options.selectedWorkspaceRoot().trim();
    const isLocalWorkspace = options.workspaceType() === "local";
    const openrindDesktopSnapshot = options.openrindDesktopServer.getSnapshot();
    ...
    if (canUseOpenrindDesktopServer) {
      const config = await openrindDesktopClient.getConfig(openrindDesktopWorkspaceId);
      return config.openrindDesktop ?? {};
    }

    if (isLocalWorkspace && isDesktopRuntime() && root) {
      return await workspaceOpenrindDesktopRead({ workspacePath: root }) as unknown as Record<string, unknown>;
    }

    return {};
  };

  const writeWorkspaceOpenrindDesktopConfigRecord = async (config: Record<string, unknown>) => {
    const root = options.selectedWorkspaceRoot().trim();
    const isLocalWorkspace = options.workspaceType() === "local";
    const openrindDesktopSnapshot = options.openrindDesktopServer.getSnapshot();
    const openrindDesktopClient = openrindDesktopSnapshot.openrindDesktopServerClient;
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopClient &&
      openrindDesktopWorkspaceId &&
      openrindDesktopSnapshot.openrindDesktopServerCapabilities?.config?.write;

    if (canUseOpenrindDesktopServer) {
      await openrindDesktopClient.patchConfig(openrindDesktopWorkspaceId, { openrindDesktop: config });
      return true;
    }

    if (isLocalWorkspace && isDesktopRuntime() && root) {
      const result = await workspaceOpenrindDesktopWrite({
        workspacePath: root,
        config: config as never,
      });
      if (!result.ok) {
        throw new Error(result.stderr || result.stdout || "Failed to write .opencode/openrind-desktop.json");
      }
      return true;
    }
  ...
  const refreshImportedCloudSkillHubs = async () => {
    try {
      const config = await readWorkspaceOpenrindDesktopConfigRecord();
  ...
  const persistImportedCloudSkillHubs = async (nextSkillHubs: Record<string, CloudImportedSkillHub>) => {
    ...
    const persisted = await writeWorkspaceOpenrindDesktopConfigRecord(nextConfig);
    if (!persisted) {
      throw new Error("Openrind Desktop server unavailable. Connect to manage imported cloud skill hubs.");
    }
  ...
  const persistImportedCloudSkills = async (nextSkills: Record<string, CloudImportedSkill>) => {
    ...
    const persisted = await writeWorkspaceOpenrindDesktopConfigRecord(nextConfig);
    if (!persisted) {
      throw new Error("Openrind Desktop server unavailable. Connect to manage imported cloud skills.");
    }
  ...
  const persistImportedCloudPlugins = async (nextPlugins: Record<string, CloudImportedPlugin>) => {
    ...
    const persisted = await writeWorkspaceOpenrindDesktopConfigRecord(nextConfig);
    if (!persisted) {
      throw new Error("Openrind Desktop server unavailable. Connect to manage imported cloud plugins.");
    }
  ...
  const upsertWorkspaceSkill = async (
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopClient &&
      openrindDesktopWorkspaceId &&
      openrindDesktopSnapshot.openrindDesktopServerCapabilities?.skills?.write;

    if (canUseOpenrindDesktopServer) {
      await openrindDesktopClient.upsertSkill(openrindDesktopWorkspaceId, {
        name,
        content,
        description,
      });
    ...
    if (isRemoteWorkspace) {
      throw new Error("Openrind Desktop server unavailable. Connect to import skills.");
    }
  ...
  const deleteWorkspaceSkill = async (name: string) => {
    ...
    if (canUseOpenrindDesktopServer) {
      await openrindDesktopClient.deleteSkill(openrindDesktopWorkspaceId, name);
      return;
    }

    if (isRemoteWorkspace) {
      throw new Error("Openrind Desktop server unavailable. Connect to remove skills.");
    }
  ...
  const writePluginWorkspaceFile = async (path: string, content: string) => {
    ...
    if (
      ...
      openrindDesktopClient &&
      openrindDesktopWorkspaceId &&
      typeof openrindDesktopClient.writeWorkspaceFile === "function"
    ) {
      await openrindDesktopClient.writeWorkspaceFile(openrindDesktopWorkspaceId, { path, content, force: true });
      return;
    }
    throw new Error("Openrind Desktop server unavailable. Connect to import plugin files into this workspace.");
  };
  ...
  async function refreshHubSkills(optionsOverride?: { force?: boolean }) {
    ...
    const canUseOpenrindDesktopServer =
      openrindDesktopSnapshot.openrindDesktopServerStatus === "connected" &&
      openrindDesktopClient &&
      openrindDesktopSnapshot.openrindDesktopServerCapabilities?.hub?.skills?.read;
    ...
    try {
      ...
      if (canUseOpenrindDesktopServer) {
        const response = await openrindDesktopClient.listHubSkills({
          repo: {
            owner: repo.owner,
            repo: repo.repo,
            ref: repo.ref,
  ...
  async function importCloudOrgPlugin(
    ...
    try {
      ...
      const token = settings.authToken?.trim() ?? "";
      const orgId = settings.activeOrgId?.trim() ?? "";
      if (!token || !orgId) throw new Error("Sign in to Openrind Desktop Cloud and choose an organization first.");
      const client = createDenClient({ baseUrl: settings.baseUrl, apiBaseUrl: settings.apiBaseUrl, token });
  ...
  async function installHubSkill(name: string): Promise<{ ok: boolean; message: string }> {
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopClient &&
      openrindDesktopWorkspaceId &&
      openrindDesktopSnapshot.openrindDesktopServerCapabilities?.hub?.skills?.install;

    if (!canUseOpenrindDesktopServer) {
      if (isRemoteWorkspace) return { ok: false, message: "Openrind Desktop server unavailable. Connect to install skills." };
      return { ok: false, message: "Hub install requires Openrind Desktop server." };
    }
    ...
    try {
      const repoOverride: OpenrindDesktopHubRepo = { owner: repo.owner, repo: repo.repo, ref: repo.ref };
      const result = await openrindDesktopClient.installHubSkill(openrindDesktopWorkspaceId, trimmed, { repo: repoOverride });
      await refreshSkills({ force: true });
  ...
  async function refreshSkills(optionsOverride?: { force?: boolean }) {
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopClient &&
      openrindDesktopWorkspaceId &&
      openrindDesktopSnapshot.openrindDesktopServerCapabilities?.skills?.read;
    ...
    if (canUseOpenrindDesktopServer) {
      ...
      try {
        setStateField("skillsStatus", null);
        const response = await openrindDesktopClient.listSkills(openrindDesktopWorkspaceId, { includeGlobal: isLocalWorkspace });
    ...
    if (!client) {
      mutateState((current) => ({
        ...current,
        skills: [],
        skillsStatus: "Openrind Desktop server unavailable. Connect to load skills.",
      }));
      return;
    }
  ...
  async function refreshPlugins(scopeOverride?: PluginScope) {
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopClient &&
      openrindDesktopWorkspaceId &&
      openrindDesktopSnapshot.openrindDesktopServerCapabilities?.plugins?.read;
    ...
    if (scope === "project" && canUseOpenrindDesktopServer) {
      mutateState((current) => ({
        ...current,
        pluginConfig: null,
        pluginConfigPath: `opencode.json (${isRemoteWorkspace ? "remote" : "openrind-desktop"} server)`,
      }));

      try {
        mutateState((current) => ({ ...current, pluginStatus: null, sidebarPluginStatus: null }));
        if (refreshPluginsAborted) return;
        const result = await openrindDesktopClient.listPlugins(openrindDesktopWorkspaceId, { includeGlobal: false });
        if (refreshPluginsAborted) return;
    ...
    if (!isLocalWorkspace && !canUseOpenrindDesktopServer) {
      mutateState((current) => ({
        ...current,
        pluginStatus: "Openrind Desktop server unavailable. Connect to manage plugins.",
        pluginList: [],
        sidebarPluginStatus: "Connect an Openrind Desktop server to load plugins.",
        sidebarPluginList: [],
      }));
      refreshPluginsInFlight = false;
      return;
  ...
  async function addPlugin(pluginNameOverride?: string) {
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopClient &&
      openrindDesktopWorkspaceId &&
      openrindDesktopSnapshot.openrindDesktopServerCapabilities?.plugins?.write;
    ...
    if (snapshot.pluginScope === "project" && canUseOpenrindDesktopServer) {
      try {
        setStateField("pluginStatus", null);
        await openrindDesktopClient.addPlugin(openrindDesktopWorkspaceId, pluginName);
    ...
    if (!isLocalWorkspace && !canUseOpenrindDesktopServer) {
      setStateField("pluginStatus", "Openrind Desktop server unavailable. Connect to manage plugins.");
      return;
    }
  ...
  async function removePlugin(pluginName: string) {
    ...
    if (snapshot.pluginScope === "project" && canUseOpenrindDesktopServer) {
      try {
        setStateField("pluginStatus", null);
        await openrindDesktopClient.removePlugin(openrindDesktopWorkspaceId, name);
  ...
  async function installSkillCreator(): Promise<{ ok: boolean; message: string }> {
    ...
    if (canUseOpenrindDesktopServer) {
      ...
      try {
        await openrindDesktopClient.upsertSkill(openrindDesktopWorkspaceId, { name: "skill-creator", content: skillCreatorTemplate });
    ...
    if (isRemoteWorkspace) {
      const message = "Openrind Desktop server unavailable. Connect to install skills.";
      setStateField("skillsStatus", message);
      return { ok: false, message };
    }
    if (!isDesktopRuntime()) {
  ...
  async function readSkill(name: string): Promise<{ name: string; path: string; content: string } | null> {
    ...
    if (canUseOpenrindDesktopServer) {
      try {
        setStateField("skillsStatus", null);
        const result = await openrindDesktopClient.getSkill(openrindDesktopWorkspaceId, trimmed, { includeGlobal: isLocalWorkspace });
    ...
    if (isRemoteWorkspace) {
      setStateField("skillsStatus", "Openrind Desktop server unavailable. Connect to view skills.");
      return null;
    }
    if (!isDesktopRuntime()) {
      setStateField("skillsStatus", translate("skills.desktop_required"));
  ...
  async function saveSkill(input: { name: string; content: string; description?: string }) {
    ...
    if (isRemoteWorkspace) {
      setStateField("skillsStatus", "Openrind Desktop server unavailable. Connect to edit skills.");
      return;
    }
    if (!isDesktopRuntime()) {
      setStateField("skillsStatus", translate("skills.desktop_required"));
  ...
  const start = () => {
    ...
    if (typeof window !== "undefined") {
      ...
      window.addEventListener("openrind-desktop-den-session-updated", onDenSessionUpdated);
      stopDenSessionListener = () => window.removeEventListener("openrind-desktop-den-session-updated", onDenSessionUpdated);
    }

    stopOpenrindDesktopSubscription = options.openrindDesktopServer.subscribe(() => {
      syncFromOptions();
    });
  ...
  const dispose = () => {
    ...
    started = false;
    abortRefreshes();
    stopOpenrindDesktopSubscription?.();
    stopOpenrindDesktopSubscription = null;
