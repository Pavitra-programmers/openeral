...
export function CreateWorkspaceModal(props: CreateWorkspaceModalProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "openrind-desktop-den-session-updated",
  ...
  const handleRemoteSubmit = async () => {
    ...
    await Promise.resolve(
      props.onConfirmRemote({
        openrindDesktopHostUrl: remoteUrl.trim(),
        openrindDesktopToken: remoteToken.trim() || null,
        directory: null,
  ...
  const handleOpenrindDesktoper = async (worker: DenWorkerSummary) => {
    ...
    try {
      ...
      const openrindDesktopUrl = tokens.openrindDesktopUrl?.trim() ?? "";
      const accessToken =
        tokens.ownerToken?.trim() || tokens.clientToken?.trim() || "";
      if (!openrindDesktopUrl || !accessToken) {
        throw new Error(translate("dashboard.error_workspace_not_ready"));
      }
      const ok = await Promise.resolve(
        props.onConfirmRemote({
          openrindDesktopHostUrl: openrindDesktopUrl,
          openrindDesktopToken: accessToken,
          openrindDesktopClientToken: tokens.clientToken?.trim() || null,
          openrindDesktopHostToken: tokens.hostToken?.trim() || null,
          directory: null,
  ...
  const handleLocalSubmit = async () => {
    // Workspaces are always the regular chat experience. Openrind Shell terminals
      ...
      {screen === "local" && props.defaultSandboxBackend ? (
        <div className="mb-4 rounded-2xl border border-dls-border bg-dls-surface p-3">
          ...
          {/* The OpenShell launch-profile picker was removed: workspaces are
              always the chat experience (profile "openrind-desktop"). Openrind Shell
