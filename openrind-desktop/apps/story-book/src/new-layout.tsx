...
import type { OpenrindDesktopServerClient } from "../../app/src/app/lib/openrind-desktop-server";
...
const localWorkspace = storyWorkspaces[0] ?? {
  ...
  displayName: "Openrind Desktop App",
  path: "~/Openrind Desktop/app",
  preset: "starter",
...
const remoteWorkspace = storyWorkspaces[1] ?? {
  ...
  remoteType: "openrind-desktop" as const,
  baseUrl: "https://worker.openrind-desktoplabs.com/opencode",
  openrindDesktopHostUrl: "https://worker.openrind-desktoplabs.com",
  openrindDesktopWorkspaceName: "Ops Worker",
  sandboxBackend: "docker" as const,
  sandboxContainerName: "openrind-desktop-ops-worker",
};
...
const mockShareFields = [
  {
    label: "Worker URL",
    value: "https://worker.openrind-desktoplabs.com/opencode",
...
export default function NewLayoutApp() {
  ...
  const [pluginList, setPluginList] = createSignal<string[]>(["@openrind/browser-mcp"]);
  ...
  const tabDescription = (tab: SettingsTab) => {
    switch (tab) {
      case "den":
        return "Manage your Openrind Desktop Cloud connection, hosted workers, and workspace access.";
      case "model":
      ...
      case "appearance":
        return "Adjust how Openrind Desktop looks across desktop, system theme, and app chrome.";
      case "updates":
      ...
      default:
        return "Connect providers, authorize folders, and control the active Openrind Desktop workspace.";
    }
  };
  ...
  const buildRouterHealth = () => ({
    ok: true,
    opencode: {
      url: "https://worker.openrind-desktoplabs.com/opencode",
      healthy: true,
      version: "0.1.0-story",
    },
    channels: {
    ...
    agent: {
      ...
      loaded: true,
      selected: "openrind-desktop-router",
    },
  });

  const mockOpenrindDesktopServerClient = {
    getConfig: async () => ({ openrindDesktop: { messaging: { enabled: messagingModuleEnabled() } } }),
    ...
    getOpenCodeRouterTelegram: async () => ({
      ...
      bot: { id: 1, username: "openrind_desktop_storybot", name: "Openrind Desktop Story Bot" },
    }),
    ...
    patchConfig: async (
      _workspaceId: string,
      payload: { openrindDesktop?: { messaging?: { enabled?: boolean } } },
    ) => {
      const enabled = payload.openrindDesktop?.messaging?.enabled;
  ...
  } as unknown as OpenrindDesktopServerClient;
  ...
  const pickMockWorkspaceFolder = async () => {
    const folders = [
      "/Users/demo/Openrind Desktop/client-foundation",
      "/Users/demo/Openrind Desktop/automation-lab",
      "/Users/demo/Openrind Desktop/starter-sandbox",
    ];
                ...
                <Show
                  ...
                  fallback={
                    <MessageList
                      ...
                      workspaceRoot="/Users/benjaminshafii/openrind-desktop-enterprise/_repos/openrind-desktop"
                    />
                  }
                >
                    ...
                    <div class="min-w-0 space-y-6">
                      ...
                      <Show when={settingsTab() === "skills"}>
                        <div class="rounded-[20px] border border-dls-border bg-dls-surface p-6 text-[14px] text-dls-secondary">
                          ...
                          Use the Openrind Desktop app shell to exercise this surface.
                        </div>
                      </Show>
                      ...
                      <Show when={settingsTab() === "messaging"}>
                        <div class="space-y-4">
                          <IdentitiesView
                            busy={false}
                            openrindDesktopServerStatus="connected"
                            openrindDesktopServerUrl="https://worker.openrind-desktoplabs.com/opencode"
                            openrindDesktopServerClient={mockOpenrindDesktopServerClient}
                            openrindDesktopReconnectBusy={false}
                            reconnectOpenrindDesktopServer={async () => true}
