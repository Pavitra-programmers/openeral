...
export function renderBundlePage({ id, rawJson, req }: { id: string; rawJson: string; req: RequestLike }): string {
  ...
  const title = bundle.name || `Openrind Desktop ${typeLabel}`;
  const description = bundle.description || buildBundleNarrative(bundle);
  const items = collectBundleItems(bundle, 8);
  const compactItem = bundle.type === "skill" ? "skill.md" : items[0]?.name || "Openrind Desktop bundle";
...
<head>
  ...
  <title>${escapeHtml(title)} - Openrind Desktop Share</title>
  <meta name="description" content="${escapeHtml(description)}" />
  <meta name="openrind-desktop:bundle-id" content="${escapeHtml(id)}" />
  <meta name="openrind-desktop:bundle-type" content="${escapeHtml(bundle.type || "unknown")}" />
  <meta name="openrind-desktop:schema-version" content="${escapeHtml(schemaVersion)}" />
  <meta name="openrind-desktop:open-in-app-url" content="${escapeHtml(openInAppDeepLink)}" />
  ...
  <style>
    @font-face {
      ...
      src: url("https://openrind-desktoplabs.com/fonts/FKRasterRomanCompact-Smooth.woff2") format("woff2");
      font-weight: 400;
      font-style: normal;
      font-display: swap;
    }
...
<body
  data-openrind-desktop-share="true"
  data-openrind-desktop-bundle-id="${escapeHtml(id)}"
  data-openrind-desktop-bundle-type="${escapeHtml(bundle.type || "unknown")}"
  data-openrind-desktop-schema-version="${escapeHtml(schemaVersion)}"
>
  <main class="shell">
    <nav class="nav">
      <a class="brand" href="/" aria-label="Openrind Desktop Share home">
        <span class="brand-mark" aria-hidden="true"></span>
        <span>openrind-desktop</span>
      </a>
      <div class="nav-links">
        <a href="https://openrind-desktoplabs.com/docs" target="_blank" rel="noreferrer">Docs</a>
        <a href="${escapeHtml(OPENRIND_DESKTOP_DOWNLOAD_URL)}" target="_blank" rel="noreferrer">Download</a>
        <a href="https://openrind-desktoplabs.com/enterprise" target="_blank" rel="noreferrer">Enterprise</a>
      </div>
      <div class="nav-actions">
        <a class="button-secondary" href="https://github.com/openrind/openrind-desktop" target="_blank" rel="noreferrer">
    ...
    <section class="hero-layout">
      <div class="hero-copy">
        ...
        <div class="hero-actions">
          <a class="button-primary" href="${escapeHtml(openInAppDeepLink)}">Open in Openrind Desktop app</a>
          <a class="button-secondary" href="https://openrind-desktoplabs.com/den" target="_blank" rel="noreferrer">Open in an Openrind Desktop den</a>
        </div>
      </div>
      ...
      <div class="hero-artifact">
        <div class="app-window">
          <div class="app-window-header">
            ...
            <div class="app-window-title">Openrind Desktop</div>
          </div>
    ...
    <section class="results-grid">
      <div class="result-card">
        <div class="step-list">
          <div class="step-row"><span class="step-bullet">01</span><span>Open the bundle in Openrind Desktop</span></div>
          <div class="step-row"><span class="step-bullet">02</span><span>Choose the destination worker</span></div>
          <div class="step-row"><span class="step-bullet">03</span><span>Happy OpenrindDesktoping!</span></div>
        </div>
      </div>
    </section>
  ...
  <script id="openrind-desktop-bundle-json" type="application/json">${escapeJsonForScript(rawJson)}</script>
  <script>
  </script>
</body>
</html>`;
