...
```bash
curl -fsSL https://raw.githubusercontent.com/openrind/openrind-desktop/dev/apps/opencode-router/install.sh | bash
```
...
- `OPENCODE_ROUTER_HEALTH_PORT` controls the port (Openrind Desktop defaults to a random free port when using `openrind-desktop`).
...
- SQLite at `~/.openrind-desktop/opencode-router/opencode-router.db` unless overridden.
- Config stored at `~/.openrind-desktop/opencode-router/opencode-router.json` (created by `opencode-router` or `pnpm -C apps/opencode-router setup`).
