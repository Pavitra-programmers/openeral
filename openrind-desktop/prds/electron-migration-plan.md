...
| OS       | Path                                                     |
| -------- | -------------------------------------------------------- |
| macOS    | `~/Library/Application Support/com.differentai.openrind-desktop/`|
| Windows  | `%APPDATA%\com.differentai.openrind-desktop\`                    |
| Linux    | `~/.config/com.differentai.openrind-desktop/`                    |

Contents (observed on a real machine):
- `openrind-desktop-workspaces.json` (Tauri's name)
- `openrind-desktop-server-state.json`
- `openrind-desktop-server-tokens.json`
- `workspaces/` subtree
...
- `~/.openrind-desktop/openrind-desktop-orchestrator/` — orchestrator daemon data
...
- `apps/desktop/src-tauri/tauri.conf.json` →
  `endpoints: ["https://github.com/openrind/openrind-desktop/releases/latest/download/latest.json"]`
...
app.setPath(
  "userData",
  path.join(app.getPath("appData"), "com.differentai.openrind-desktop"),
);
```

```yaml
# apps/desktop/electron-builder.yml
appId: com.differentai.openrind-desktop   # (currently com.differentai.openrind-desktop.electron)
```
...
- First Electron launch finds the Tauri-written `openrind-desktop-server-*.json`
...
async function readWorkspaceState() {
  const legacy = path.join(userData, "openrind-desktop-workspaces.json"); // Tauri
...
1. A single new command `migrate_to_electron()` in the Tauri shell that:
   - Downloads the matching Electron installer from the same GitHub Release
     (`Openrind Desktop-0.12.0-mac-<arch>.dmg`, `.exe`, `.AppImage`).
...
2. A one-time prompt:
   ...
   > Openrind Desktop is moving to a new desktop engine. We'll install the new
...
latest.json says 0.12.0 is out → DMG installed in-place → Tauri v0.12.0-migrate
      ...
      ▼
migrate_to_electron():
  download Openrind Desktop-0.12.0-electron-mac.dmg from same release
  codesign --verify ✓
  open installer, schedule Tauri quit
      │
      ▼
Installer replaces the .app bundle
  appId = com.differentai.openrind-desktop (same as Tauri)
  Launchpad slot + Dock pin preserved, no duplicate "Openrind Desktop" icon
      │
      ▼
Openrind Desktop Electron v0.12.0 first launch
  app.setPath("userData", .../com.differentai.openrind-desktop) points at the
  Tauri-written folder → tokens, workspaces, orchestrator state already there
  rename openrind-desktop-workspaces.json → workspace-state.json (once)
      │
      ▼
...
- `Release App` workflow:
  - Replaces `tauri build` with `pnpm --filter @openrind/desktop package:electron`.
...
- **Bundle-identifier drift**. If Electron `appId` is different from Tauri,
  ...
  `com.differentai.openrind-desktop`.
...
- `openrind-desktop.react.activeWorkspace` — last selected workspace
- `openrind-desktop.react.sessionByWorkspace` — per-workspace last session
- `openrind-desktop.server.list` — multi-server list
- `openrind-desktop.server.active` — selected server
- `openrind-desktop.server.urlOverride` — server override
- `openrind-desktop.server.token` — server token
...
- Electron main exposes `openrind-desktop:migration:read` and
  `openrind-desktop:migration:ack` IPC handlers; preload bridges them under
  `window.__OPENRIND_DESKTOP_ELECTRON__.migration`.
...
2. **This PR** — "migration engine". Unifies `appId` + `userData` path to
   Tauri's, adds `openrind-desktop-workspaces.json` → `workspace-state.json`
...
3. **Last Tauri release v0.12.0** — ships:
   ...
   - a one-time prompt ("Openrind Desktop is moving to a new engine — install?")
