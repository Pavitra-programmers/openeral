...
Status: draft
Owner: src-opn
Scope: Openrind Desktop server + Den web + OpenCode integration
...
- **Installing a bundle into a workspace** means writing each member primitive to its **native `.opencode/` path** via the Openrind Desktop server's existing per-workspace endpoints. The `reload-watcher` then propagates to running sessions.
...
|---|---|---|---|
| Plugin manifest | `.claude-plugin/plugin.json` | **None** (no manifest file) | ❌ — Openrind Desktop-only DB concept |
| Marketplace catalog | `.claude-plugin/marketplace.json` | **None** | ❌ — Openrind Desktop-only DB concept |
...
```markdown
---
description: Run the Openrind Desktop release flow
---

You are running the Openrind Desktop release flow in this repo.
Arguments: `$ARGUMENTS`
```
...
Bundle
  id
  org_id
  slug                       # "openrind-desktop-release-kit"
  name                       # "Openrind Desktop Release Kit"
  description
  version                    # "2.3.1"
  icon                       # emoji or URL
...
WorkspaceInstallation
  ...
  org_id
  bundle_id
  workspace_id               # the Openrind Desktop workspace (worker + path)
...
Materialization steps (all executed server-side against the Openrind Desktop server API — not direct FS access):
...
The `reload-watcher` on the server picks up each file write and emits `ReloadEvent`s keyed by `workspaceId`. OpenCode-running sessions pick up skills/commands/agents hot; plugin code and MCP changes require a new session (reload-watcher already handles this via `openrind-desktop.json` → `reload.auto`).
...
- A tiny **Openrind Desktop skill** running inside the user's workspace maintains a **local SQLite** DB that mirrors the subset of the remote DB relevant to that workspace (primitives + installed bundles).
...
```
 User            Den web             Den API             Provider           Openrind Desktop server
  │                 │                  │                    │                    │
  │─ click Connect ─▶                  │                    │                    │
...
```
 User            Den web           Den API                         Openrind Desktop server                           Disk
  │                 │                │                                 │                                      │
  │─ click Install ▶│                │                                 │                                      │
  ...
  │                 │                │                                 │                                      │
  │                 │  OpenCode session:                                                                      │
  │                 │    auto-reload if openrind-desktop.json.reload.auto, else toast "reload available"              │
```
...
| C. Ship a universal runner | Publish one npm package `@openrind/claude-hooks-runtime` that reads JSON hooks from a well-known path (`.opencode/openrind-desktop/hooks/*.json`) and registers them once. Installing any Bundle with JSON hooks just drops files in that path. | High upfront (one-time infrastructure) but zero per-Bundle cost thereafter. | v2 — best long-term |
...
Closes the loop with the ecosystem: an authored Bundle can be pushed back to a Git repo as a Claude-compatible marketplace tree so other Openrind Desktop (or Claude Code) users can ingest it.
...
Ingesting an exported tree back into Openrind Desktop MUST produce semantically-identical Primitives (same `contentHash` per primitive, same Bundle membership). This is a tested invariant — see [Test strategy](#test-strategy).
...
openrind-desktop bundle list [--installed]
openrind-desktop bundle show <bundle-slug>
openrind-desktop bundle install <bundle-slug> [--scope workspace|user|org]
openrind-desktop bundle uninstall <bundle-slug>
openrind-desktop bundle publish <bundle-slug> --to github:owner/repo [--ref main]

openrind-desktop primitive list [--kind=skill|agent|…]
openrind-desktop primitive show <name> --kind=<kind>
openrind-desktop primitive create --kind=skill --file=./my-skill.md
```
...
An **Openrind Desktop skill** wraps the CLI and surfaces it to agents:

```yaml
---
name: openrind-desktop-plugin-manager
description: |
  Install, uninstall, and discover Openrind Desktop plugins (bundles).
...
- **Materialization**: mock the Openrind Desktop server API; assert correct endpoint + body per primitive kind; assert `appliedPrimitiveDigests` is recorded.
...
1. **Agents mutation endpoint**: Openrind Desktop server has `/workspace/:id/{skills,commands,plugins,mcp}` but **no dedicated agents endpoint**. Options: (a) add `POST /workspace/:id/agents`, (b) use the generic `POST /workspace/:id/files/content` for agents, (c) bundle agents under plugins. Recommend (a) for parity.
