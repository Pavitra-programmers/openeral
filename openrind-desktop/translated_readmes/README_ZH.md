...
我们为 AI 智能体配备您团队已经在使用的工具，并让它们从您的行为中学习。您使用 Openrind Desktop 越多，工具之间的连接就越紧密，积累的知识就越多，能够自动化的工作块就越大。

Openrind Desktop 是 opencode 的最简单界面。双击，选择一个文件夹，即可立即获得三大核心优势：
...
Openrind Desktop 现在位于 `packages/app`（UI）和 `packages/desktop`（桌面外壳）中。
...
- 在**主机模式**中，Openrind Desktop 启动：
  - `opencode serve --hostname 127.0.0.1 --port <free-port>`
  - 以您选择的项目文件夹作为进程工作目录。
在主机模式下，Openrind Desktop 直接在您的计算机上后台启动 OpenCode 服务器。
当您选择项目文件夹时，Openrind Desktop 使用该文件夹在本地运行 OpenCode 并将桌面 UI 连接到它。
...
插件是扩展 OpenCode 的**原生**方式。Openrind Desktop 现在通过从技能选项卡读取和写入 `opencode.json` 来管理它们。
