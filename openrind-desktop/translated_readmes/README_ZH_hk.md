...
我們為 AI 智能體配備您團隊已經在使用的工具，並讓它們從您的行為中學習。您使用 Openrind Desktop 越多，工具之間的連接就越緊密，積累的知識就越多，能夠自動化的工作塊就越大。

Openrind Desktop 是 opencode 的最簡單介面。雙擊，選擇一個資料夾，即可立即獲得三大核心優勢：
...
Openrind Desktop 圍繞一個核心理念設計：讓您可以輕鬆地將智能體工作流程作為可重複的、產品化的流程進行交付。
...
## 快速開始
在此處下載安裝包：https://github.com/openrind/openrind-shell/releases（或按照下面的說明從源代碼安裝）
...
Openrind Desktop 現在位於 `packages/app`（UI）和 `packages/desktop`（桌面外殼）中。
...
- 在**主機模式**中，Openrind Desktop 啟動：
  - `opencode serve --hostname 127.0.0.1 --port <free-port>`
  - 以您選擇的專案資料夾作為進程工作目錄。
在主機模式下，Openrind Desktop 直接在您的電腦後台啟動 OpenCode 伺服器。
當您選擇專案資料夾時，Openrind Desktop 使用該資料夾在本地運行 OpenCode 並將桌面 UI 連接到它。
...
插件是擴展 OpenCode 的**原生**方式。Openrind Desktop 現在通過從技能選項卡讀取和寫入 `opencode.json` 來管理它們。
...
有興趣在您的組織中使用 Openrind Desktop？我們很樂意聽取您的意見 — 請發送郵件至 [ben@openrindlabs.com](mailto:ben@openrindlabs.com) 與我們討論您的用例。

## 致謝與鳴謝

Openrind Desktop 基於並借鑑了 [Different AI](https://github.com/different-ai) 的開源項目 [Openwork](https://github.com/different-ai/openwork)。我們對 Openwork 原作者和社群貢獻者建立的開源基石表示誠摯的感謝與致敬。
